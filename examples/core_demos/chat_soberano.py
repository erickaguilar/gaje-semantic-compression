...
def main():
    parser = argparse.ArgumentParser(
        description="🧬 GAJE PROTOCOL: CHAT SOBERANO (Silver Adult)"
    )
    parser.add_argument(
        "--model",
        type=str,
        default="models/production/silver_adult_steel.gaje",
    ...
    parser.add_argument(
        "--spiking", action="store_true", help="Use Neuromorphic Spiking Inference"
    )
    parser.add_argument(
        "--prompt",
        type=str,
    ...
    if not os.path.exists(model_path):
        ...
        for p in [
            "models/production/silver_adult_steel.gaje",
    ...
    if args.spiking:
        print("🧠 MODO NEUROMÓRFICO (SPIKING) ACTIVADO")
    print("-" * 60)
    chat_history = []
