    ...
    if mode == "event":
        ...
        for _ in range(max_tokens):
            ...
            for tick in range(10):
                ...
                final_spikes = [
                    s for s in output_spikes if s.target_layer_id >= len(layers)
                ]
                if final_spikes:
        ...
        for _ in range(max_tokens):
            ...
            if s0:
                ...
                if s1:
                    ...
                    next_id = (
                        sorted(s2, key=lambda x: x[1], reverse=True)[0][0]
                        if s2
                        else (input_id + 1) % config["vocab_size"]
                    )
                else:
                    next_id = (input_id + 1) % config["vocab_size"]
            else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="models/checkpoints/smg1_overnight_gold_evolved.gaje",
    )
