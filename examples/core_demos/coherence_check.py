...
def calculate_ppl(llm, text):
    ...
    if hasattr(
        tokens, "ids"
    ):  # En caso de que sea un tokenizador de la lib 'tokenizers'
        tokens = tokens.ids
    ...
    for i in range(n_tokens - 1):
        ...
        if (i + 1) % 10 == 0:
            print(
                f"    [~] Progreso: {i+1}/{n_tokens-1} tokens evaluados...", flush=True
            )
...
def main():
    ...
    for forced_rope in [original_rope, 10000.0, 100000.0, 1000000.0]:
        ...
        test_text = (
            "El sistema GAJE permite la compresión semántica de modelos de lenguaje."
        )
