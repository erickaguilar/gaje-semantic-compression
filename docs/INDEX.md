...
*   **[FROZEN_BODY_CAUSAL_AB_PROTOCOL.md](docs/research/FROZEN_BODY_CAUSAL_AB_PROTOCOL.md)**: Protocolo A/B causal (PROPUESTO) — aísla la causa del congelamiento del cuerpo: herencia de maestros (descartada por evidencia previa) vs régimen de entrenamiento vs discretización Q4_0. 4 brazos con control ya ejecutado; incluye validación del flujo reordenado especializar-primero/cuantizar-después.
...
*   **[WASM_BRAINSTEM_PLAN.md](docs/plans/WASM_BRAINSTEM_PLAN.md)**: Plan GAJE-WASM — compilación del motor a WebAssembly (auditoría de dependencias, abstracción `TensorSource`, crate `gaje-wasm`) y diseño de la API sensorio-motora que convierte al motor en "tronco encefálico" del modelo: aferencia unificada, bus de acciones y ciclo autonómico con consolidación `.gmem`. 6 fases con umbrales de decisión, gates de paridad WASM↔nativo y camino de soberanía máxima (tokenizador embebido en Rust + target WASI sin JS).
*   **[MEMORY_EPOCHS_PLAN.md](docs/plans/MEMORY_EPOCHS_PLAN.md)**: Plan Épocas de Memoria — conocimiento flexible sobre cuerpo Q4_0 congelado: `.gmem` v2 (linaje en los 40 B reservados del header), épocas inmutables con manifiesto de métricas, gate de promoción por harness generativo, rollback exacto y evolución confinada a la capa de memoria. 4 fases con umbrales; integra el ciclo autonómico del tronco WASM.
*   **[ZERO_ORDER_NATIVE_TRAINING_PLAN.md](docs/plans/ZERO_ORDER_NATIVE_TRAINING_PLAN.md)**: Plan de Entrenamiento Nativo de Orden Cero — SPSA discreto sobre asignaciones de centroide (solo forward passes, sin STE ni backprop): operador de perturbación antitética parametrizado por semillas, currículo híbrido crecimiento+refinamiento y escalera de nacimiento nativo micro→5M→32M. 4 fases con umbrales; conecta el ES refutado con su versión viable (2 evaluaciones/paso, precedente MeZO).
