...
print(f"[*] Prefill forward loop ({len(input_ids)} tokens): {prefill_ms:.2f} ms ({prefill_ms/len(input_ids):.2f} ms/tok)")
...
print(f"[*] generate_native_py: {gen_ms:.2f} ms for {n_gen} tokens ({gen_ms/n_gen:.2f} ms/tok)")
