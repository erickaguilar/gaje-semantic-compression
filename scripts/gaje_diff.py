...
def run_gaje_diff():
    ...
    print("=" * 70)
    print(f"{'Capa':<12} | {'Nombre Capa':<20} | {'CosSim':<10} | {'L2 Norm (HF)':<14} | {'L2 Norm (GAJE)':<14} | {'Estado':<8}")
    print("-" * 70)
    ...
    for p_idx, tid in enumerate(input_ids):
        ...
        if p_idx == len(input_ids) - 1:
            ...
            for l_idx, block in enumerate(gaje_llm.blocks):
                ...
                status = "✔ PERFECT" if cos_sim > 0.99 else ("⚠️ DERIVA" if cos_sim > 0.5 else "❌ FALTAN BIAS")
                print(f"Bloque {l_idx:02d}    | {f'TransformerBlock.{l_idx}':<20} | {cos_sim:.6f}   | {hf_norm:<14.4f} | {rust_norm:<14.4f} | {status}")
    ...
    print("-" * 70)
    print(f"LM Head      | Output Vocabulary Logits | {logit_cossim:.6f}   | {np.linalg.norm(hf_logits):<14.4f} | {np.linalg.norm(rust_logits):<14.4f} | {'✔ PERFECT' if logit_cossim > 0.99 else '❌ FALTAN BIAS'}")
    print("=" * 70)
