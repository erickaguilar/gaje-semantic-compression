...
def calculate_coherence_v095(model_path, dataset_path, num_samples=100):
    ...
    for idx, text in enumerate(lines[:num_samples]):
            ...
            if s0:
                ...
                if s1:
                    ...
                    if s2:
                        ...
                        if total_tokens % 100 == 0: print(f"      [Debug] L2 SILENCIO | S1 count: {len(s1)}")
                else:
                    if total_tokens % 100 == 0: print(f"      [Debug] L1 SILENCIO | S0 count: {len(s0)}")
            else:
                if total_tokens % 100 == 0: print(f"      [Debug] L0 SILENCIO | Input ID: {input_id}")
        ...
        if (idx + 1) % 10 == 0:
            ...
            print(f"    [~] Muestra {idx+1}/{num_samples} | Hits: {hits}/{total_tokens} | Acc: {current_acc:.2f}%")
    ...
    print("-" * 50)
    print(f"✅ RESULTADOS COHERENCIA (v0.9.5-alpha)")
...
if __name__ == "__main__":
    calculate_coherence_v095(
        "models/checkpoints/smg1_debug.gaje",
