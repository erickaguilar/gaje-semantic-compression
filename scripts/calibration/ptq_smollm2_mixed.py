...
def run_ptq_pipeline():
    ...
    print("🚀 Iniciando Exportación Final PTQ Mixed-Bit: SmolLM2-135M")
    log_ram("Inicio")
    ...
    print(
        f"✅ {len(layers)} capas para genomizar, {len(other_tensors)} tensores directos."
    )

    # 2. Inicializar Writer
    print(
        f"[Debug] GajeDatabaseWriter attrs: {dir(dna_semantic_compression.GajeDatabaseWriter)}"
    )
    ...
    for name, tensor in tqdm(layers.items()):
        ...
        (
            ...
            w_f32.tobytes(), block_size, anchor_rate, bit_depth
        )
        ...
        if base_name == "token_embd":
            ...
            batch.write_tensor(
                "lm_head.centroids", np.array(centroids, dtype=np.float32).tobytes()
            )
