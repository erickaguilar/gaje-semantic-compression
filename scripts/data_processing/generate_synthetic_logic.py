...
def generate_synthetic_logic_es(count=5000):
    ...
    temas = [
        ...
        "los rasgos (traits)",
        "la concurrencia nativa",
        "la compresión genómica",
        "los tensores de 2 bits",
        "la perplejidad diferencial",
    ]
    mecanismos = [
        "el verificador de préstamos",
        "el conteo de referencias",
        "la seguridad en tiempo de compilación",
        "la cuantización de centroides",
        "la matriz de adyacencia relacional",
        "el paralelismo con Rayon",
        "la optimización SIMD",
    ]
    beneficios = [
        "evitar fugas de memoria",
        "garantizar la seguridad de hilos",
        "maximizar la eficiencia en ARM",
        "reducir el semantic drift",
        "lograr una inferencia zero-gil",
    ]
    alternativas = [
        "punteros crudos",
        "recolección de basura",
        "copia profunda",
        "memoria compartida mutable",
...
if __name__ == "__main__":
    ...
    print(
        f"✅ Lógica sintética guardada en: {output_path} ({len(data.encode('utf-8')) / 1024:.2f} KB)"
    )
