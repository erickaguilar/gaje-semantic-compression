...
def export_gaje_flat():
    ...
    out_path = os.path.join(PROJECT_ROOT, "models", "production", "qwen2_0_5b_4bit.gaje.flat")

    os.makedirs(os.path.dirname(out_path), exist_ok=True)

    print(f"🚀 Exportador Flat Zero-Copy v0.9.7 (.gaje.flat)")
    ...
    tokenizer_str = tokenizer.backend_tokenizer.to_str() if hasattr(tokenizer, "backend_tokenizer") else ""
    ...
    def process_and_add_layer(name, layer):
        dna_b = bytes(layer.linear.database) if hasattr(layer.linear, "database") else bytes(layer.dna_database)
        c_b = np.array(layer.linear.centroids if hasattr(layer.linear, "centroids") else layer.dna_centroids, dtype=np.float32).tobytes()
    ...
    for i in range(n_blocks):
        ...
        for norm_suffix in ["attn_norm", "ffn_norm"]:
            ...
            if g_norm_key in tensors_by_name:
                ...
                norm_b = np.frombuffer(t.data, dtype=np.float32).tobytes() if t.tensor_type == gguf.GGMLQuantizationType.F32 else np.frombuffer(t.data, dtype=np.float16).astype(np.float32).tobytes()
        ...
        # 1. Fused QKV
        w_q = get_tensor_f32_matrix(tensors_by_name[f"blk.{i}.attn_q.weight"], n_head, head_dim, is_q_k=False)
        w_k = get_tensor_f32_matrix(tensors_by_name[f"blk.{i}.attn_k.weight"], n_head_kv, head_dim, is_q_k=False)
        ...
        # 4. FFN Down
        process_layer_data(p + "ffn_down", tensors_by_name[f"blk.{i}.ffn_down.weight"], bit_depth=4, bias_obj=None)
    ...
    struct.pack_into("<4sIIIQQQQ", header_bin, 0,
                     magic,
                     version,
                     flags,
                     num_tensors,
