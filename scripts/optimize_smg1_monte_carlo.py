...
def evaluate_fitness(layers, dataset, tokenizer, centroides, thresholds, sample_size=50):
    ...
    for i, layer in enumerate(layers):
        l = engine.GajeNeuromorphicLayer(layer.num_neurons, layer.weights_per_neuron, thresholds[i], 0.8)
...
def main():
    parser = argparse.ArgumentParser(description="🎲 GAJE: Monte Carlo Centroid Optimization (Phase 2)")
    parser.add_argument("--model", type=str, default="models/checkpoints/smg1_overnight_gold.gaje")
    ...
    print(f"🎲 Iniciando Fase 2: Optimización Monte Carlo")
    ...
    best_score = evaluate_fitness(base_layers, lines, tokenizer, best_centroides, best_thresholds, sample_size=args.sample_size)
    ...
    for i in range(args.iterations):
        ...
        score = evaluate_fitness(base_layers, lines, tokenizer, candidate_c, candidate_t, sample_size=args.sample_size)
        
        if score > best_score:
            best_score = score
            best_centroides = candidate_c
            best_thresholds = candidate_t
            print(f"🔥 Iter {i+1}: Score: {best_score:.4f} | C: {best_centroides} | T: {best_thresholds}")
