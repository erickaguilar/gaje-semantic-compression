...
def coherence_test_monte_carlo():
    ...
    database = np.random.randint(0, 256, (out_features, in_features // 4), dtype=np.uint8)
    ...
    def evaluate_coherence(lyr, inp, target, label):
        ...
        cos_sim = np.dot(output, target) / (np.linalg.norm(output) * np.linalg.norm(target))
    ...
    # --- Evaluación Baseline ---
    mse_b, cos_b, ent_b = evaluate_coherence(layer, input_vec, target_output, "BASELINE (2-BIT)")
    ...
    start = time.time()
    layer.monte_carlo_refine(input_vec.tolist(), target_output.tolist(), iterations=2000, noise_scale=0.02)
    duration = time.time() - start
    
    # --- Evaluación Post-Monte Carlo ---
    print(f"[*] Refinamiento completado en {duration:.2f}s\n")
    mse_m, cos_m, ent_m = evaluate_coherence(layer, input_vec, target_output, "MONTE CARLO (OPTIMIZED)")
