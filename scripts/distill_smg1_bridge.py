...
def main():
    ...
    parser.add_argument("--dataset", type=str, default="data/datasets/tiny_shakespeare.txt")
    parser.add_argument("--epochs", type=int, default=10)
    parser.add_argument("--lr", type=float, default=0.5)
    parser.add_argument("--output", type=str, default="models/checkpoints/smg1_fixed_coherence.gaje")
    ...
    vocab_size = len(teacher.tokenizer)
    dim_latent = 256
    dim_logic = 128
    ...
    student_layers = [
        engine.GajeNeuromorphicLayer(dim_latent, vocab_size, 0.4, 0.8),
        engine.GajeNeuromorphicLayer(dim_logic, dim_latent, 0.4, 0.8),
        engine.GajeNeuromorphicLayer(vocab_size, dim_logic, 0.4, 0.8)
    ]
    ...
    for epoch in range(1, args.epochs + 1):
        hits = 0
        tokens_processed = 0
        l0_spikes, l1_spikes, l2_spikes = 0, 0, 0
        ...
        for text_idx, text in enumerate(lines):
            ...
            for t in range(len(tokens) - 1):
                ...
                # RESET DE ESTADO (Aislamiento de Tokens)
                for layer in student_layers:
                    layer.reset_potentials()

                # Capa 0: Asociación de Entrada
                l0_deltas = [0.0] * dim_latent
                offset = (input_id * 16) % dim_latent
                for i in range(16): l0_deltas[(offset + i) % dim_latent] = 1.0
                ...
                # Capa 1: Lógica (Puente)
                l1_deltas = [0.0] * dim_logic
                offset_l1 = (input_id * 8) % dim_logic
                for i in range(8): l1_deltas[(offset_l1 + i) % dim_logic] = 1.0
            ...
            if (text_idx + 1) % 1000 == 0:
                print(f"    [~] Época {epoch}: {text_idx + 1}/{len(lines)} | S0:{l0_spikes//1000}k S1:{l1_spikes//1000}k S2:{l2_spikes}", flush=True)

        if tokens_processed > 0:
            accuracy = (hits / tokens_processed) * 100
            print(f"🔥 Época {epoch}/{args.epochs} | Precisión: {accuracy:.2f}% | S2 Total: {l2_spikes} | T:{time.time()-start_time:.1f}s", flush=True)

    print("\n✅ DESTILACIÓN COMPLETADA")
    save_smg1_model(args.output, student_layers, vocab_size, dim_latent, dim_logic, centroides)
