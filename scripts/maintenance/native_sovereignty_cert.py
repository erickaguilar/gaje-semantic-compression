...
def run_sovereignty_test(model_path):
    ...
    if hasattr(token_id, "ids"): token_id = token_id.ids
    token_id = token_id[0]
    ...
    for i in range(5):
        ...
        if i > 0 and not np.array_equal(distributions[i], distributions[0]):
            is_stable = False
            print(f"    [!] Inestabilidad en iteración {i}: {top_k} vs {distributions[0]}")
    ...
    print("-" * 70)
    coherence = len(response_tokens) > 0 and not any(t == response_tokens[0] for t in response_tokens[1:4])
    if coherence:
