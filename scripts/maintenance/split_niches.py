...
CONNECTORS = [
    r"\by\b", r"\bo\b", r"\bpero\b", r"\bporque\b", r"\bauunque\b", 
    r"\bentonces\b", r"\bademás\b", r"\bsin embargo\b", r"\bluego\b",
    r"\bpor lo tanto\b", r"\bpor eso\b", r"\bsi bien\b", r"\baunque\b"
]

# Sufijos verbales para Nicho C (Español)
VERB_SUFFIXES = [
    "amos", "iste", "ieron", "ará", "ería", "ando", "ido", 
    "emos", "imos", "aste", "asteis", "arán", "erán", "irán",
...
def split_niches():
    ...
    for line in lines:
        ...
        words = line.split()
        has_verb_suffix = any(any(word.lower().endswith(s) for s in VERB_SUFFIXES) for word in words)
        
        # Heurística Nicho A (Sintaxis Base)
        # Oraciones cortas, declarativas, sin demasiados conectores
        is_short = 4 <= len(words) <= 15
        is_declarative = line.endswith(".") and not (line.endswith("?") or line.endswith("!"))
