...
def create_mosaic():
        ...
        with open(sources["cultural"], "r", encoding="utf-8") as f:
            for line in f:
                ...
                if len(line) > 50 and is_spanish(line):
                    ...
                    if len(cultural_lines) >= 10000:
                        break  # Límite para esta versión
    ...
    print(
        f"📊 Fuentes cargadas: Cultural({len(cultural_lines)}), Técnico({len(technical_lines)}), Chat({len(chat_lines)})"
    )
