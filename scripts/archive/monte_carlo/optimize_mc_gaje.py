...
def main():
    ...
    parser.add_argument(
        "--arch", type=str, default="qwen2", help="Arquitectura a optimizar"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=300,
        help="Número de generaciones de Monte Carlo",
    )
    parser.add_argument(
        "--out",
        type=str,
        default="models/checkpoints/mc_optimized_qwen",
        help="Directorio de salida",
    )
    args = parser.parse_args()

    print(
        f"🎲 Iniciando Optimización Global Monte Carlo para {args.arch} ({args.iterations} generaciones)"
    )
    config = get_config(args.arch)
    llm = GenomicLLM(model_path=None, num_blocks=args.blocks, config=config)

    print(
        "[*] Organismo inicializado (Born-Genomic). Comenzando optimización de centroides..."
    )
    ...
    print("    -> Optimizando Token Embeddings...")
    improved, mse = optimize_layer_mc(
        llm, llm.embeddings, "token_embd", iterations=args.iterations
    )
    if improved:
        layers_optimized += 1
    ...
    for i, block in enumerate(llm.blocks):
        ...
        p = f"blk.{i}."
        # Atención
        imp, _ = optimize_layer_mc(
            llm, block.attn_layer.q_gen, p + "attn_q", iterations=args.iterations
        )
        layers_optimized += int(imp)
        imp, _ = optimize_layer_mc(
            llm, block.attn_layer.k_gen, p + "attn_k", iterations=args.iterations
        )
        layers_optimized += int(imp)
        imp, _ = optimize_layer_mc(
            llm, block.attn_layer.v_gen, p + "attn_v", iterations=args.iterations
        )
        layers_optimized += int(imp)
        imp, _ = optimize_layer_mc(
            llm, block.attn_layer.w_o, p + "attn_output", iterations=args.iterations
        )
        layers_optimized += int(imp)
        # FFN
        imp, _ = optimize_layer_mc(
            llm, block.gate_gen, p + "ffn_gate", iterations=args.iterations
        )
        layers_optimized += int(imp)
        imp, _ = optimize_layer_mc(
            llm, block.up_gen, p + "ffn_up", iterations=args.iterations
        )
        layers_optimized += int(imp)
        imp, _ = optimize_layer_mc(
            llm, block.w_down, p + "ffn_down", iterations=args.iterations
        )
        layers_optimized += int(imp)
    ...
    improved, mse = optimize_layer_mc(
        llm, llm.lm_head, "lm_head", iterations=args.iterations
    )
    if improved:
        layers_optimized += 1
    ...
    print(
        f"✅ Optimización MC completada en {duration:.2f}s. Capas mejoradas: {layers_optimized}"
    )
