...
def monte_carlo_centroid_search(weights_f32, iterations=5000, noise_scale=0.1):
    ...
    print(f"   Mejora Relativa: {((simulate_gaje_quantization(weights_f32, base_centroids) - best_mse) / simulate_gaje_quantization(weights_f32, base_centroids)) * 100:.2f}%")
...
if __name__ == "__main__":
    ...
    np.random.seed(42)
    dummy_weights = np.random.normal(0, 0.02, 1024) + np.random.uniform(-0.01, 0.01, 1024)
