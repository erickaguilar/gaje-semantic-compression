...
def apply_iqat_calibration(model_path, calibration_prompts, lr=0.005):
    print(f"🚀 Iniciando Calibración IQAT Quirúrgica para: {os.path.basename(model_path)}")
    ...
    for b_idx in range(student.n_blocks):
        ...
        if p + "ffn_gate.weight" not in tensor_map:
            print(f"      [!] Bloque {b_idx} no tiene pesos SwiGLU estándar. Saltando refinamiento FFN...")
            continue
        ...
        ffn_norm_weight = np.frombuffer(tensor_map[p + "ffn_norm.weight"].data, dtype=np.float32)
        ...
        for prompt in calibration_prompts:
            ...
            for i, tid in enumerate(input_ids):
                ...
                student.rust_llm.blocks[b_idx].refine_ffn(x_norm, teacher_swiglu_target, lr)
