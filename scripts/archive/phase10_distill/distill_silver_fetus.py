...
class ConsensusDistiller:
        ...
        for t_name, teacher in self.teachers.items():
            ...
            for logits in logits_seq:
                ...
                if s > 0:
                    ...
                    student_probs = np.ones_like(student_probs) / len(student_probs)
    ...
    def distill_step(self, text, trainer, lr):
        ...
        for i in range(n_steps):
            ...
            logits_s, h_norm = self.student.rust_llm.forward_with_hidden(
                tokens[i], False
            )
...
def main():
    ...
    arch = ArchConfig(
        name=name, version="0.9.8", tokenizer_id=tokenizer_path, rope_base=1000000.0
    )
    config = ModelConfig(
        config=arch,
        n_embd=n_embd,
