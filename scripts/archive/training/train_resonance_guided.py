...
def main():
    parser = argparse.ArgumentParser(
        description="🧬 GAJE Graph-Guided Resonance Training (Phase 4.1)"
    )
    parser.add_argument(
        "--model",
        type=str,
        default="models/checkpoints/gold_embryo.gaje",
        help="Ruta al modelo base",
    )
    parser.add_argument(
        "--topology",
        type=str,
        default="models/core/topology_es.json",
        help="Ruta al mapa de topología",
    )
    parser.add_argument(
        "--dataset",
        type=str,
        default="data/datasets/dataset_es.txt",
        help="Ruta al dataset (ES)",
    )
    parser.add_argument(
        "--tokenizer",
        type=str,
        default="models/core/tokenizer.json",
        help="Ruta al tokenizador",
    )
    parser.add_argument(
        "--epochs", type=int, default=10, help="Épocas de entrenamiento"
    )
    parser.add_argument("--lr", type=float, default=0.001, help="Learning Rate")
    parser.add_argument(
        "--output",
        type=str,
        default="models/checkpoints/gold_embryo_guided.gaje",
        help="Ruta de guardado",
    )
    ...
    if os.path.exists(args.topology):
        ...
        print(
            f"⚠️ Advertencia: Topología no encontrada en {args.topology}. Continuando sin guía."
        )
    ...
    if os.path.exists(args.tokenizer):
        ...
        print("❌ Error: Tokenizador no encontrado.")
        return
    ...
    print("[*] Iniciando fase de resonancia guidada...")
