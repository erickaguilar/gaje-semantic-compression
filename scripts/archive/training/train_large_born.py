...
def main():
    parser = argparse.ArgumentParser(
        description="🧬 GAJE PROTOCOL: BORN-GENOMIC TRAINING (LARGE)"
    )
    parser.add_argument(
        "--name", type=str, default="GajeExpert-v1", help="Name of the model"
    )
    parser.add_argument(
        "--blocks",
        type=int,
    ...
    parser.add_argument(
        "--embd", type=int, default=512, help="Embedding dimension (default: 512)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--dataset",
        type=str,
        default="data/datasets/dataset_entrenamiento.txt",
        help="Path to dataset",
    )
    args = parser.parse_args()

    print("🧬 GAJE PROTOCOL: BORN-GENOMIC LARGE TRAINING")
    print("=" * 60)
    ...
    llm = GenomicLLM(
        model_path=None, num_blocks=args.blocks, config=config, n_embd=args.embd
    )
