...
def rope_unit_test():
    ...
    rope.base = rope_base
    rope.inv_freq = 1.0 / (
        rope.base ** (torch.arange(0, head_dim, 2).float() / head_dim)
    )
    ...
    ArchitectureConfig(name="test", rope_base=rope_base, rope_style="split")

    # Create a minimal attention layer
    dna_semantic_compression.GenomicAttention(
    ...
    q_np.tolist()
    q_np.tolist()  # Same for simplicity
    q_np.tolist()
