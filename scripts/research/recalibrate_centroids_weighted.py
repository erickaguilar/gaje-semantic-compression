    ...
    for _ in range(max_iters):
        ...
        for j in range(k):
            ...
            if np.any(mask):
                ...
                new_centroids[j] = np.sum(data[mask] * weights[mask]) / np.sum(
                    weights[mask]
                )
            else:
...
def recalibrate_organism(model_path, corpus_path):
    print("🧬 [RE-CALIBRACIÓN] Alineamiento Global de Centroides (Weighted Fisher)")
    ...
    print("-" * 50)
    print("📊 COMPARATIVA DE CENTROIDES (ε-net)")
    ...
    print("📉 IMPACTO EN DISTORSIÓN (Fisher-weighted MSE)")
...
if __name__ == "__main__":
    recalibrate_organism(
        "models/production/silver_adult_sovereign.gaje",
        "data/datasets/curated_1mb_flash.txt",
    )
