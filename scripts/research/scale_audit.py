...
def scale_audit():
    ...
    print(
        f"Original Max: {np.max(w_original):.4f}, Min: {np.min(w_original):.4f}, Mean Abs: {np.mean(np.abs(w_original)):.4f}"
    )
    print(
        f"Reconstructed Max: {np.max(w_rec):.4f}, Min: {np.min(w_rec):.4f}, Mean Abs: {np.mean(np.abs(w_rec)):.4f}"
    )
