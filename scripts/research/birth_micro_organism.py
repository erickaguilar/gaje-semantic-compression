...
def birth_sequence():
    ...
    print(f"[*] Fase 3: Inyectando Anclas en dimensiones de alta entropía (> {avg_entropy:.2f})...")
    ...
    try:
        index = gaje_core.GajeIndex(config['hidden_dim'], [-1.0, -0.2, 0.2, 1.0])
        print(f"    [+] GajeIndex (Toroidal) inicializado para {config['hidden_dim']} dimensiones.")
    except Exception as e:
        print(f"    [!] Error al inicializar GajeIndex: {e}")

    print(f"\n✅ Nacimiento Completado: {model_path}")
    print(f"[*] Tamaño estimado en disco: ~8.5 MB")
