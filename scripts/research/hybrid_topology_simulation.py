...
def run_simulation(size=100000, mc_iterations=3000):
    print("=" * 70)
    print(f"🧬 SIMULADOR HÍBRIDO DE TOPOLOGÍAS Y CUANTIZACIÓN v1.1-Alpha")
    ...
    std = np.std(weights)
    print(f"[*] Estadísticas del vector original:")
    ...
    results["1. Lineal Clásico"] = {
        "mse": mse_lin, "cos": cos_lin, "entropy": ent_lin, "time": t_lin, "centroids": c_lin
    }
    ...
    results["2. Algebraico Q(ζ₁₆)"] = {
        "mse": mse_alg, "cos": cos_alg, "entropy": ent_alg, "time": t_alg, "centroids": c_alg
    }
    ...
    results["3. Circular Complejo"] = {
        "mse": mse_comp, "cos": cos_comp, "entropy": ent_comp, "time": t_comp, "centroids": c_comp
    }
    
    # 4. Híbrido Monte Carlo + Q(zeta_16)
    t0 = time.time()
    rec_mc, idx_mc, c_mc, mse_mc = monte_carlo_hybrid_optimization(weights, mc_iterations)
    t_mc = time.time() - t0
    cos_mc = calculate_cosine_similarity(weights, rec_mc)
    ent_mc = calculate_entropy(idx_mc, 4)
    results["4. Híbrido Monte Carlo"] = {
        "mse": mse_mc, "cos": cos_mc, "entropy": ent_mc, "time": t_mc, "centroids": c_mc
    }
    
    # 📊 Imprimir Reporte Comparativo
    print(f"{'METODOLOGÍA':<22} | {'MSE (Menor es mejor)':<20} | {'SIMILITUD COSENO':<18} | {'ENTROPÍA (Máx 2.0)':<18} | {'TIEMPO (s)':<10}")
    print("-" * 97)
    for name, data in results.items():
        print(f"{name:<22} | {data['mse']:>20.8f} | {data['cos']:>18.6f} | {data['entropy']:>18.4f} | {data['time']:>10.4f}")
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# 📊 Reporte de Simulación: Topologías Híbridas de Cuantización (Fase 5.0)\n\n")
        f.write("**Fecha:** 31 de mayo de 2026  \n")
        f.write(f"**Dataset de Simulación:** Pesos simulados de capa densa ($N={size}$ parámetros, $\\mu=0.002, \\sigma=0.015$).\n\n")
        f.write("## 1. Tabla Comparativa de Rendimiento\n\n")
        f.write("| Metodología | MSE de Reconstrucción | Similitud Coseno | Entropía de Codificación (Uso) | Tiempo de Cómputo (s) |\n")
        f.write("| :--- | :---: | :---: | :---: | :---: |\n")
        for name, data in results.items():
            f.write(f"| {name} | `{data['mse']:.8f}` | `{data['cos']:.6f}` | `{data['entropy']:.4f}` | `{data['time']:.4f}s` |\n")
        ...
        f.write("2. **Entropía de Codificación:** La entropía mide qué tan equitativamente se usan los 4 estados de 2 bits (Adenina, Citosina, Guanina, Timina). Una entropía cercana a `2.0` (máxima teórica para 2 bits) indica que no hay saturación o subutilización de códigos. La topología circular y la híbrida muestran una excelente distribución, previniendo el colapso atencional.\n")
        f.write("3. **Soberanía Algebraica:** El uso de raíces ciclotómicas estructuradas ofrece un anclaje matemático rígido que previene la deriva semántica del gradiente continuo, mientras que las mutaciones Monte Carlo proporcionan la adaptabilidad fina necesaria durante la crianza.\n")

    print(f"\n[+] Reporte guardado con éxito en: {report_path}")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Simulador de Topologías de Cuantización")
    parser.add_argument("--size", type=int, default=100000, help="Tamaño del vector de pesos")
    parser.add_argument("--iterations", type=int, default=3000, help="Iteraciones de optimización Monte Carlo")
