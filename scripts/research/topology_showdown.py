...
def run_showdown(gaje_path, topo_rust_path, topo_es_path):
    print("🏔️  INICIANDO EL DUELO: The Topology Showdown (Fase 4.0)")
    ...
    for label, data in results.items():
        print(
            f"{label:<15} | {data['base']:>12.2f} | {data['topo']:>12.2f} | {data['gain']:>9.2f}%"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gaje", type=str, default="models/checkpoints/gold_embryo.gaje"
    )
    parser.add_argument(
        "--topo_rust", type=str, default="models/core/topology_rust.json"
    )
