...
def prune_tokenizer(input_path, output_path, target_vocab_size=32768):
    ...
    pruned_vocab = {
        token: tid for token, tid in vocab.items() if tid < target_vocab_size
    }
    ...
    if "added_tokens" in data:
        data["added_tokens"] = [
            t for t in data["added_tokens"] if t["id"] < target_vocab_size
        ]
...
if __name__ == "__main__":
    prune_tokenizer(
        "models/core/tokenizer_bak.json", "models/core/tokenizer.json", 32768
    )
