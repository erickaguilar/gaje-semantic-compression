...
def reference_block_forward_verbose(x, weights):
    ...
    k_exp = np.repeat(k.reshape(n_head_kv, head_dim), n_groups, axis=0).flatten()
...
def block_audit():
    ...
    # A. Check Projections
    x_norm_gaje = np.array(dna_semantic_compression.rms_norm_py(x_in.tolist(), weights['attn_norm'].tolist(), 1e-6))
