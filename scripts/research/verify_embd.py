...
def verify_embeddings(gguf_path, gaje_path):
    ...
    centroids_full = np.frombuffer(
        reader_gaje.read_tensor(f"{gaje_prefix}.centroids"), dtype=np.float32
    )

    # Dequantize row 1
    n_embd = 576
    block_size = 32
    n_embd // block_size  # 18
    ...
    # Cos Sim
    cos_sim = np.dot(w_orig_row, w_recon_row) / (
        np.linalg.norm(w_orig_row) * np.linalg.norm(w_recon_row)
    )
...
verify_embeddings(
    "models/gguf/smollm2-135m-f16.gguf", "models/production/smollm2_mixed_v1.gaje"
)
