...
def run_identity_test():
    ...
    print(f"🚀 Iniciando TEST DE IDENTIDAD (0-Loss) - FIX")
    ...
    print(f"[~] Construyendo pesos de identidad para {layer_name} (Size: {n_elements})...")
    ...
    config = {
        ...
        "n_embd": 576, "n_head": 9, "n_blocks": 30, "vocab_size": 49152,
    }
    ...
    batch.write_tensor(f"{base_name}.centroids", np.array(all_centroids, dtype=np.float32).tobytes())
    
    # Anchor buffer dummy
    anchors_buf = b"GAJE" + struct.pack("<I", 0) + struct.pack("<Q", 0) + struct.pack("<Q", 0)
    ...
    centroids_read = np.frombuffer(reader_gaje.read_tensor(f"{base_name}.centroids"), dtype=np.float32)
