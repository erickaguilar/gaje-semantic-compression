...
def verify_quantization(gguf_path, gaje_path):
    print("🔬 [VERIFICACIÓN] GGUF vs GAJE")
    ...
    cos_sim = np.dot(w_orig.flatten(), w_recon) / (
        np.linalg.norm(w_orig) * np.linalg.norm(w_recon)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gaje", type=str, default="models/production/smollm2_mixed_v1.gaje"
    )
