...
TIERS = [
    {
        "name": "micro_organism",
        "label": "Peldaño 1: Micro-Organismo (1.5M)",
        "preset": "micro_organism",
        "file": "birth_micro.flat",
        "k": 16,
        "epochs": 2,
    },
    {
        "name": "embryo_5m",
        "label": "Peldaño 2: Embrión 5M (5.2M)",
        "preset": "embryo_5m",
        "file": "birth_5m.flat",
        "k": 32,
        "epochs": 2,
    ...
    {
        "name": "silver_adult_32m",
        "label": "Peldaño 3: Organismo 32M (32M)",
        "preset": "silver_adult_32m",
        "file": "birth_32m.flat",
        "k": 64,
        "epochs": 2,
    },
...
for tier in TIERS:
    ...
    cmd_init = [cli_bin, "--init", out_path, "--preset", tier["preset"]]
    ...
    print(
        f"[*] Entrenamiento de Orden Cero: SPSA Discreto sobre Q4_0 (k={tier['k']}, épocas={tier['epochs']})..."
    )
    t_train = time.time()
    final_loss = trainer.fit_zero_order(
        llm.rust_llm, NUTRITION_DATASET, epochs=tier["epochs"], k_coords=tier["k"]
    )
    train_duration = time.time() - t_train

    print(
        f"✅ Entrenamiento completado en {train_duration:.2f}s | Loss final: {final_loss:.4f}"
    )

    # 4. Evaluación de Gate Generativo (Verificación de Estabilidad y No-Degeneración)
    print("[*] Verificando Gate Generativo del peldaño...")
    ...
    if not is_degenerate and len(generated_tokens) > 0:
        print(
            f"✅ GATE APROBADO para {tier['name']}: Generación estable, variada y 0% colapso monótono."
        )
    else:
