...
def run_needle_test(model_path, tokenizer_path, context_lengths=[128, 256, 512, 1024]):
    print(f"🧪 Needle in a Haystack - Silver Fetus Edition (Phase 5.0) 🧪")
    ...
    if os.path.exists(topo_path):
        ...
        print(f"[*] Topología CAM inyectada.")
    ...
    for length in context_lengths:
        ...
        print(f"    - Resultado: {'✅ ÉXITO' if success else '❌ FALLO'} | {duration:.2f}s")
    ...
    with open("docs/reports/NEEDLE_VALIDATION_20260525.md", "w") as f:
        f.write(f"# 🧪 Reporte de Validación: Needle in a Haystack (Stage 5)\n\n")
        f.write(f"**Modelo:** Silver Fetus (12.3 MB)\n")
        f.write(f"**Fecha:** 25 de mayo de 2026\n\n")
        f.write(f"| Contexto (tokens) | Resultado | Respuesta | Latencia |\n")
        f.write(f"| :--- | :--- | :--- | :--- |\n")
        for r in results:
            res_str = "✅" if r["success"] else "❌"
            f.write(f"| {r['length']} | {res_str} | {r['response']} | {r['time']:.2f}s |\n")
