...
def run_phase_3_1(gaje_path, context_lengths=[512, 1024, 2048]):
    print("🚀 Iniciando Fase 3.1: La Aguja en el Pajar (Needle in a Haystack)")
    ...
    print("\n" + "=" * 60)
    print("📊 RESULTADO FINAL FASE 3.1")
    for r in results:
        status = "✅" if r["found"] else "❌"
        print(
            f"  - Contexto {r['length']:>4}: {status} | Tiempo: {r['time']:.2f}s | Resp: {r['response'][:30]}..."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Longitudes de contexto separadas por comas",
    )
