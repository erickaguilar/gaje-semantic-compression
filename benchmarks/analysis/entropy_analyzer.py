...
class EntropyValidator:
    ...
    def __init__(self, model_path):
        ...
        self.embd_tensor = next(
            t for t in self.reader.tensors if t.name == "token_embd.weight"
        )
        print(
            f"    [+] Tensor de Referencia: {self.embd_tensor.name} (Type: {self.embd_tensor.tensor_type})"
        )
        ...
        W_sample = self.dequantize_q8_0(
            self.embd_tensor.data[:sample_rows], self.hidden_dim
        )
        ...
        for i in range(0, self.embd_tensor.shape[0], chunk_size):
            ...
            W_chunk = self.dequantize_q8_0(
                self.embd_tensor.data[i:end], self.hidden_dim
            )
            ...
            dna_db, _, a_bin = dna_semantic_compression.genomize_f32_native(
                W_chunk.tobytes(),
                32,
                -1.0,  # -1.0 para mantener anclas (opcional)
            )
            ...
            if i % 25000 == 0:
                print(
                    f"    [~] Progreso: {i}/{self.embd_tensor.shape[0]} tokens genomizados..."
                )
        ...
        self.engine = dna_semantic_compression.GenomicLinear(
            ...
            self.centroids
            * (
                self.embd_tensor.shape[0] * (self.hidden_dim // 32)
            ),  # Repetir centroides por bloque
...
if __name__ == "__main__":
    model_path = (
        "/data/data/com.termux/files/home/models/gguf/qwen2-0_5b-instruct-fp16.gguf"
    )
