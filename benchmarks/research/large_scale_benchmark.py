...
def run_scale(scale_info):
    ...
    print(
        f"[*] Ejecutando búsqueda Flat ADC (Query: {current_sample_size})...",
        flush=True,
    )
    latencies = []
    ...
    for i in range(current_sample_size):
        ...
        _ = dna_semantic_compression.dna_similarity_search_adc(
            q.tolist(), db_dna, CENTROIDS
        )
...
def main():
    ...
    print(
        "\n🚀 Resultados finales guardados en benchmarks/large_scale_results.json",
        flush=True,
    )
