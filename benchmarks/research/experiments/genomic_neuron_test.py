...
def test_genomic_neuron():
    ...
    for i in range(out_features):
        ...
        row_dna = W_dna[i]
        weights_rec = dna_semantic_compression.dequantize_embedding(
            row_dna, in_features, centroids
        )
    ...
    if cosine_sim > 0.95:
        print(
            "🚀 CONCLUSIÓN: La neurona genómica es viable para destilación de alta fidelidad."
        )
    else:
