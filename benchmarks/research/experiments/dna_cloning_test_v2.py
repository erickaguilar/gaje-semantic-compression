...
def explain_and_test():
    ...
    cos_std = np.dot(original_weights, std_weights) / (
        np.linalg.norm(original_weights) * np.linalg.norm(std_weights)
    )
    ...
    cloned_weights[anchors_idx] = apply_quant(
        original_weights[anchors_idx], centroids_anchors
    )

    cos_cloned = np.dot(original_weights, cloned_weights) / (
        np.linalg.norm(original_weights) * np.linalg.norm(cloned_weights)
    )

    print("\n" + "=" * 45)
    print("📊 VALIDACIÓN DE TU HIPÓTESIS")
    print("-" * 45)
