...
def dequantize_variant_a(tensor):
    ...
    flat = dna_semantic_compression.dequantize_q8_0_native(
        tensor.data.tobytes(), out_f, in_f
    )
...
def diagnostic_test():
    ...
    norm_w = next(
        t for t in reader.tensors if "output_norm.weight" in t.name
    ...
    for name, variant_fn in [
        ("Variant A (In=shape[0])", dequantize_variant_a),
