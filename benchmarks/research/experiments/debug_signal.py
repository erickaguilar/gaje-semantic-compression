...
def debug_signal_fidelity():
    ...
    cos_sim = np.dot(w_original, w_rec) / (np.linalg.norm(w_original) * np.linalg.norm(w_rec))
    
    print(f"--- Estadísticas (K-Means 1D) ---")
    print(f"MSE:      {mse:.8f}")
    print(f"Cos Sim:  {cos_sim:.6f}")
    print(f"Rel Error: {np.sum(np.abs(w_original - w_rec)) / np.sum(np.abs(w_original)):.4%}")
