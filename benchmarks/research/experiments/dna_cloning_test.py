...
def analyze_anchors():
    ...
    cos_std = np.dot(weights_flat, reconstructed_std) / (
        np.linalg.norm(weights_flat) * np.linalg.norm(reconstructed_std)
    )
    ...
    reconstructed_cloned[anchors_flat] = quantize(
        weights_flat[anchors_flat], centroids_high
    )

    mse_cloned = np.mean((weights_flat - reconstructed_cloned) ** 2)
    cos_cloned = np.dot(weights_flat, reconstructed_cloned) / (
        np.linalg.norm(weights_flat) * np.linalg.norm(reconstructed_cloned)
    )

    # 4. RESULTADOS
    print("\n" + "=" * 45)
    print("📊 RESULTADO DE LA CLONACIÓN")
    print("-" * 45)
    print("MÉTRICA         | ESTÁNDAR (2b) | CLONADO (Mix)")
    print(
        f"Error (MSE)     | {mse_std:.6f}      | {mse_cloned:.6f} {'🔥' if mse_cloned < mse_std else ''}"
    )
    print(
        f"Similitud Cos   | {cos_std:.4f}        | {cos_cloned:.4f} {'🚀' if cos_cloned > cos_std else ''}"
    )
    print("-" * 45)

    improvement = (cos_cloned - cos_std) / (1 - cos_std) * 100
    print(f"💡 GANANCIA DE FIDELIDAD: {improvement:.2f}%")
    print("📦 IMPACTO EN ESPAÑA: +0.1 bits/dim (Insigificante)")
    print("=" * 45)
