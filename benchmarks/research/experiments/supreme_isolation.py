...
def calculate_top_prediction(model, text, disable_attn=False, disable_ffn=False):
    ...
    for block in model.blocks:
        ...
        if not disable_ffn:
            ...
            # LOGGING
            print(
                f"      DEBUG FFN: x_norm={np.linalg.norm(x_norm):.4f}, gate_max={np.max(gate):.4f}, up_max={np.max(up):.4f}, out_norm={np.linalg.norm(ffn_out):.4f}"
            )
...
def run_supreme_isolation():
    ...
    res = calculate_top_prediction(
        model, test_phrase, disable_attn=True, disable_ffn=True
    )
    print(f"   [1] Identidad (Sin Attn, Sin FFN): '{res}'")

    # Caso 2: Sólo Atención
    res = calculate_top_prediction(
        model, test_phrase, disable_attn=False, disable_ffn=True
    )
    print(f"   [2] Sólo Atención:                 '{res}'")

    # Caso 3: Sólo FFN
    res = calculate_top_prediction(
        model, test_phrase, disable_attn=True, disable_ffn=False
    )
    print(f"   [3] Sólo FFN:                      '{res}'")

    # Caso 4: Bloque Completo
    res = calculate_top_prediction(
        model, test_phrase, disable_attn=False, disable_ffn=False
    )
