...
def simulate_ivf_pq(db_vecs, query_vec, k=10, m=8, k_centroids=256):
    ...
    for i in range(m):
        ...
        db_pq_sim[:, start:end] = db_vecs[:, start:end] + np.random.normal(
            0, 0.05, (db_vecs.shape[0], dim_per_m)
        )
...
def run_comparison():
    ...
    codebook = train_genomic_codebook(
        db_vecs, output_path="benchmarks/logs/comp_codebook.json"
    )
    db_dna = [
        dna_semantic_compression.quantize_pq(v.tolist(), codebook["thresholds"])
        for v in db_vecs
    ]
    ...
    for name, func in methods.items():
        ...
        for _ in range(num_queries):
            ...
            if name == "GAJE Protocol (DNA)":
                res = dna_semantic_compression.dna_similarity_search_adc(
                    query.tolist(), db_dna, codebook["centroids"]
                )
