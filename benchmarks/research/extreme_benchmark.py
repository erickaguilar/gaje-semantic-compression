...
def run_scale(scale_info):
    ...
    print(
        f"[*] Generando {n:,} vectores y cuantizando a Rust (Incremental)...",
        flush=True,
    )
    ...
    for i in range(0, n, chunk_size):
        ...
        if i % 100_000 == 0 and i > 0:
            gc.collect()
            print(
                f"    [~] Progreso: {i:,}/{n:,} embeddings en Rust Index...", flush=True
            )
    ...
    print(
        f"[+] Memoria DNA en Rust: {est_mem:.2f} MB (Cero overhead de Python)",
        flush=True,
    )

    # 2. Búsqueda Flat ADC (Directo en Rust Index)
    print(
        f"[*] Ejecutando búsqueda Flat ADC desde Rust Index (Query: {sample_size})...",
        flush=True,
    )
    latencies = []
    ...
    # 3. Recall Check
    print("[*] Validando Recall@1...", flush=True)
...
def main():
    ...
    with open("benchmarks/logs/extreme_scale_results.json", "w") as f:
        json.dump(results, f, indent=4)
    print(
        "\n🚀 Resultados guardados en benchmarks/logs/extreme_scale_results.json",
        flush=True,
    )
