...
def run_phase3_validation():
    ...
    # 1. Load Real Data
    db_vecs, words = load_glove_subset(glove_path, num_vectors=5000)
    dims = db_vecs.shape[1]
    top_k = 10

    # 2. Train Codebook on Real Distribution
    codebook = train_genomic_codebook(db_vecs, output_path="benchmarks/glove_codebook.json")
    ...
    db_dna = [dna_semantic_compression.quantize_pq(v.tolist(), thresholds) for v in db_vecs]
    ...
    for _ in range(num_queries):
        ...
        # GAJE ADC Results
        adc_results = dna_semantic_compression.dna_similarity_search_adc(query_vec.tolist(), db_dna, centroids)
    ...
    print("-" * 60)
    print(f"RESULTADOS FINALES (GloVe-100d):")
    print(f"🎯 Precisión (Recall@10): {avg_accuracy:.2f}%")
    print("-" * 60)
    
    if avg_accuracy >= 80:
        print("🏆 OBJETIVO ALCANZADO: El protocolo es altamente confiable en datos reales.")
    elif avg_accuracy >= 60:
        print("📈 EXCELENTE PROGRESO: Superior a cualquier método de compresión binaria estándar.")
    else:
        print("💡 NOTA: GloVe tiene una estructura densa. Se recomienda probar con sub-espacios más pequeños (PQ Fine-tuning).")
