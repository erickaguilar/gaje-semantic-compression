...
def benchmark_search_adc():
    ...
    db_vecs = np.array(
        [
            normalize(v)
            for v in np.random.normal(0, 1, (num_records, dims)).astype(np.float32)
        ]
    )
...
def benchmark_linear_anchors():
    ...
    anchors_bytes = (
        np.random.normal(0, 0.1, (out_features * n_embd,)).astype(np.float16).tobytes()
    )
    dna = bytes(
        np.random.randint(
            0, 256, (out_features * n_embd // 4,), dtype=np.uint8
        ).tolist()
    )
    centroids = [0.0] * (out_features * n_embd // block_size * 4)

    layer = core.GenomicLinear(
        dna, anchors_bytes, centroids, out_features, n_embd, block_size
    )
    x = np.random.normal(0, 1.0, (n_embd,)).astype(np.float32).tolist()

    for _ in range(5):
        _ = layer.forward(x, False)  # Warmup

    iterations = 50
    start_time = time.time()
    for _ in range(iterations):
        _ = layer.forward(x, False)
...
def benchmark_full_block():
    ...
    def get_linear(in_f, out_f):
        dna = bytes(
            np.random.randint(0, 256, (out_f * in_f // 4,), dtype=np.uint8).tolist()
        )
    ...
    block = core.RustGenomicBlock(
        0,
        attn,
        get_linear(n_embd, n_embd),
        get_linear(n_embd, n_embd),
        get_linear(n_embd, n_embd),
        get_linear(n_embd, n_embd),
        get_linear(n_embd, ffn_hidden),
        get_linear(n_embd, ffn_hidden),
        get_linear(ffn_hidden, n_embd),
        [1.0] * n_embd,
        1e-6,
        "swiglu",
    )
    ...
    print(
        f"📉 Velocidad estimada (24 bloques): {1000 / (avg_latency * 24):.2f} tokens/s"
    )
