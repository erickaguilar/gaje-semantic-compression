...
def run_level_1_audit(model_path, context_lengths=[4096, 16384, 32768, 65536]):
    ...
    print("[*] Reto: Needle In A Haystack (NIHY) con Recirculación de Fase")
    ...
    for length in context_lengths:
        ...
        if hasattr(needle_tokens, "ids"):
            needle_tokens = needle_tokens.ids
        ...
        print(
            f"[*] Inyectando {len(full_context)} tokens en el espacio de fase toroidal..."
        )
        ...
        if hasattr(q_tokens, "ids"):
            q_tokens = q_tokens.ids
    ...
    for r in audit_results:
        ...
        print(
            f"{r['length']:<12} | {status:<10} | {r['latency']:>8.2f}s | {r['response'][:40]}"
        )

    overall_score = (
        sum(1 for r in audit_results if r["found"]) / len(audit_results) * 100
    )
    print("-" * 70)
    print(f"CERTIFICACIÓN SCORE: {overall_score:.2f}%")
    print(
        f"ESTADO DE SELLO:     {'🏆 GRADE-A CERTIFIED' if overall_score == 100 else '❌ DENIED'}"
    )
    print("=" * 70)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model", type=str, default="models/production/silver_adult_clean_v1.gaje"
    )
