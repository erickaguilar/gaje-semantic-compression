...
def main():
    ...
    print(
        f"decode : {args.tokens} tokens, modo={'greedy' if args.greedy else 'sampler'}"
    )
    print("=" * 72)
    ...
    rss = process_rss_mb()
    print(
        f"[{'cold-start mmap':<28}] {load_s * 1000:>9.1f} ms  RSS={rss:.1f} MB"
        if rss
        else f"[{'cold-start mmap':<28}] {load_s * 1000:>9.1f} ms"
    )
    ...
    print("\n-- PREFILL (prompt) --")
    print(
        f"  total : {p_arr.sum():.1f} ms | media {p_arr.mean():.2f} ms/tok | "
    ...
    print(
        f"  tokens : {n_decode} | total {total_d:.1f} ms | "
        f"media {d_arr.mean():.2f} ms/tok ({1000.0 / d_arr.mean():.2f} tok/s)"
    )
    print(
        f"  p50 {np.median(d_arr):.2f} | p95 {np.percentile(d_arr, 95):.2f} | "
    ...
    if n_decode >= 8:
        ...
        if delta > 1.3:
            print(
                "  >> El decode DEGRADA con el largo de contexto -> patrón O(n^2) / mem-bandwidth."
            )
        else:
            print(
                "  >> Decode estable frente al contexto -> descarta recomputación O(n^2)."
            )
    ...
    print("=" * 72)
    print(
        "SUGERENCIA: reproduce el 0.2 tok/s con este benchmark. Si aqui da \n"
