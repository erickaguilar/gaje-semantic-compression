...
def standard_binary_quantize(val):
    if val < -0.5: return 0b00 # 0
    if val < 0.0:  return 0b01 # 1
    if val < 0.5:  return 0b10 # 2
    return 0b11               # 3
...
def gray_code_quantize(val):
    ...
    if val < -0.5: return 0b00 # 0
    if val < 0.0:  return 0b01 # 1
    if val < 0.5:  return 0b11 # 3 (Adjacent to 01 by 1 bit)
...
def run_gray_comparison():
    ...
    test_values = np.linspace(-1.0, 1.0, 100)
