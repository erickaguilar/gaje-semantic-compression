...
impl DistillationGraph {
    pub fn new() -> Self {
        Self {
            ...
            student_tokenizers: HashMap::new(),
            edges: Vec::new(),
            next_id: 0,
        }
    }
    ...
    pub fn add_edge(
        ...
        if !self.teachers.contains_key(&teacher) {
            return Err(format!("teacher {:?} no existe", teacher));
        }
        if !self.students.contains_key(&student) {
            return Err(format!("student {:?} no existe", student));
        }
        if !(0.0..=1.0).contains(&alpha) {
            return Err("alpha 0..1".into());
        }
        self.edges.push(DistillEdge {
            teacher,
            student,
            alpha,
            temperature,
    ...
    pub fn consensus_for_student(&self, student: NodeId, text: &str) -> Vec<Vec<f32>> {
        ...
        let vocab = self
            .students
            .get(&student)
            .map(|s| s.lock().unwrap().lm_head.out_features)
            .unwrap_or(49152);
    ...
    pub fn distill_edge(&self, edge: &DistillEdge, text: &str, lr: f32) -> Result<f32, String> {
        let student_arc = self
            .students
            .get(&edge.student)
            .ok_or("student no encontrado")?
            .clone();
        let mut student = student_arc.lock().unwrap();
        let tok = self
            .student_tokenizers
            .get(&edge.student)
            .ok_or("tokenizer no encontrado")?
            .clone();
        ...
        #[cfg(feature = "gpu")]
        {
            if let Some(d) = crate::compute::gpu::pipeline::GpuOnlineDistiller::try_new_global(
                32,
                edge.temperature,
                edge.alpha,
            ) {
        ...
        if let Some(t) = self.teachers.get(&edge.teacher) {
            council.add_teacher(t.clone());
        }
            ...
            for edge in &self.edges {
                ...
                for txt in texts {
                    match self.distill_edge(edge, txt, lr) {
                        ...
                        Err(e) => {
                            eprintln!("  edge {:?}->{:?} err: {}", edge.teacher, edge.student, e)
                        }
                    }
                }
                if cnt > 0 {
                    println!(
                        "  edge {:?}->{:?} avg loss {:.4} (alpha={} temp={} w={})",
                        edge.teacher,
                        edge.student,
                        edge_loss / cnt as f32,
                        edge.alpha,
                        edge.temperature,
                        edge.weight
                    );
    ...
    pub fn describe(&self) {
        println!(
            "DistillationGraph: {} nodos ({} teachers, {} students), {} aristas",
            self.nodes.len(),
            self.teachers.len(),
            self.students.len(),
            self.edges.len()
        ...
        for n in self.nodes.values() {
            println!("  {:?} {} {:?}", n.id, n.name, n.kind);
        }
        for e in &self.edges {
            println!(
                "  {:?}->{:?} α={} T={} w={}",
                e.teacher, e.student, e.alpha, e.temperature, e.weight
            );
        }
    }
}

impl Default for DistillationGraph {
    fn default() -> Self {
        Self::new()
    }
}
