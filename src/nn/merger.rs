...
pub fn merge_genomic_models(models: &[GenomicLLM]) -> Result<GenomicLLM, String> {
    ...
    println!("[*] Fusionando {} organismos genómicos mediante promediado de centroides...", models.len());
    ...
    for b_idx in 0..merged.blocks.len() {
        ...
        let q_sources: Vec<&GenomicLinear> = models.iter().map(|m| &m.blocks[b_idx].q_gen).collect();
        average_centroids(&mut target_block.q_gen, &q_sources);
        
        let k_sources: Vec<&GenomicLinear> = models.iter().map(|m| &m.blocks[b_idx].k_gen).collect();
        average_centroids(&mut target_block.k_gen, &k_sources);
        
        let v_sources: Vec<&GenomicLinear> = models.iter().map(|m| &m.blocks[b_idx].v_gen).collect();
        ...
        let gate_sources: Vec<&GenomicLinear> = models.iter().map(|m| &m.blocks[b_idx].gate_gen).collect();
        average_centroids(&mut target_block.gate_gen, &gate_sources);
        
        let up_sources: Vec<&GenomicLinear> = models.iter().map(|m| &m.blocks[b_idx].up_gen).collect();
        average_centroids(&mut target_block.up_gen, &up_sources);
        
        let down_sources: Vec<&GenomicLinear> = models.iter().map(|m| &m.blocks[b_idx].w_down).collect();
