...
impl GenomicLLM {
    ...
    pub fn clear_cache_core(&mut self) { for block in &mut self.blocks { block.clear_cache_core(); } }
    ...
    pub fn train_on_sequence_core(&mut self, tokens: Vec<usize>, lr: f32) -> Result<f32, String> {
        ...
        for i in 0..tokens.len() - 1 {
            ...
            let mut sum_e = 0.0f32; let mut exps = vec![0.0f32; logits.len()];
            for j in 0..logits.len() { let e = (logits[j] - max_l).exp(); exps[j] = e; sum_e += e; }
            let prob = (exps[target] / sum_e).max(1e-12); total_loss -= prob.ln();
            let mut d_logits = vec![0.0f32; logits.len()]; for j in 0..logits.len() { d_logits[j] = exps[j] / sum_e; }
            d_logits[target] -= 1.0;
    ...
    pub fn apply_mutation_core(&mut self, layer_name: &str, delta: Vec<f32>, undo: bool) -> Result<(), String> {
        if layer_name == "token_embd" { return self.embeddings.apply_mutation_core(delta, undo); }
        if layer_name == "lm_head" { return self.lm_head.apply_mutation_core(delta, undo); }
        if layer_name.starts_with("blk.") {
            let parts: Vec<&str> = layer_name.split('.').collect();
            if parts.len() >= 3 {
                let b_idx: usize = parts[1].parse().map_err(|e| format!("Invalid block: {}", e))?;
    ...
    pub fn apply_weighted_layer_mutation_core(&mut self, layer_name: &str, delta: Vec<f32>, weight: f32) -> Result<(), String> {
        if layer_name == "token_embd" { return self.embeddings.apply_weighted_mutation_core(delta, weight); }
        if layer_name == "lm_head" { return self.lm_head.apply_weighted_mutation_core(delta, weight); }
        if layer_name.starts_with("blk.") {
            let parts: Vec<&str> = layer_name.split('.').collect();
            if parts.len() >= 3 {
                let b_idx: usize = parts[1].parse().map_err(|e| format!("Invalid block index: {}", e))?;
                if b_idx < self.blocks.len() {
                    let b = &mut self.blocks[b_idx];
                    match parts[2] {
                        ...
                        "ffn_gate" => return b.gate_gen.apply_weighted_mutation_core(delta, weight),
    ...
    pub fn mutate_layer_core(&mut self, layer_name: &str, scale: f32) -> Result<Vec<f32>, String> {
        if layer_name == "token_embd" { return self.embeddings.mutate_random_core(scale); }
        if layer_name == "lm_head" { return self.lm_head.mutate_random_core(scale); }
        if layer_name.starts_with("blk.") {
            let parts: Vec<&str> = layer_name.split('.').collect();
            if parts.len() >= 3 {
                let b_idx: usize = parts[1].parse().map_err(|e| format!("Invalid block index: {}", e))?;
