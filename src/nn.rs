...
pub struct GenomicAttention {
    pub q_database: Vec<u8>,
    pub k_database: Vec<u8>,
    pub v_database: Vec<u8>,
    pub centroids: Vec<f32>,
    pub stride: usize,
    pub n_head: usize,
    pub n_head_kv: usize,
...
impl GenomicAttention {
    #[new]
    #[allow(clippy::too_many_arguments)]
    #[pyo3(signature = (q, k, v, centroids, stride, n_head, n_head_kv, _rmsnorm_weight=Vec::new(), _eps=1e-6))]
    pub fn new(
        q: Vec<u8>,
        k: Vec<u8>,
        v: Vec<u8>,
        ...
        for h in 0..self.n_head {
            ...
            for &byte in q_dna {
                for s in 0..4 {
                    ...
                    query[q_start + dims] =
                        unsafe { dot_product_neon(&input_vector, &vec![val; n_embd]) };
                    dims += 1;
                }
            }
        }
