...
pub struct Calibration { pub cpu_ms: f32, pub gpu_ms: f32, pub ratio: f32 }
...
impl CalibratedScheduler {
    pub fn new() -> Self { Self { ewma_cpu: 400.0, ewma_gpu: 380.0, last_choice: true, alpha: 0.3 } }
    ...
    pub fn calibrate_once_text(&mut self, text: &str) -> Calibration {
        ...
        Calibration { cpu_ms, gpu_ms, ratio: gpu_ms/cpu_ms }
    }
    ...
    pub fn choose(&mut self, cal: Calibration) -> bool {
        ...
        let next = if ratio < 0.8 { true } else if ratio > 1.2 { false } else { self.last_choice };
        self.last_choice = next;
        next
    }
    pub fn describe(&self) -> String { format!("EWMA cpu={:.1}ms gpu={:.1}ms last={}", self.ewma_cpu, self.ewma_gpu, if self.last_choice{"GPU"}else{"CPU"}) }
}
impl Default for CalibratedScheduler { fn default() -> Self { Self::new() } }
