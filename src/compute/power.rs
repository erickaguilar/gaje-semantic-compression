...
impl PowerManager {
    ...
    pub fn detect() -> Self {
        ...
        for i in 0..32 { // Límite razonable de núcleos
        ...
        match unique_freqs.len() {
            ...
            2 => {
                ...
                clusters.insert(CpuCluster::Little, freqs.get(&unique_freqs[0]).unwrap().clone());
                clusters.insert(CpuCluster::Big, freqs.get(&unique_freqs[1]).unwrap().clone());
            }
            _ => {
                // Tri-cluster (Ej: 4+3+1 como en el output previo)
                clusters.insert(CpuCluster::Little, freqs.get(&unique_freqs[0]).unwrap().clone());
                clusters.insert(CpuCluster::Medium, freqs.get(&unique_freqs[1]).unwrap().clone());
                clusters.insert(CpuCluster::Big, freqs.get(&unique_freqs[unique_freqs.len() - 1]).unwrap().clone());
            }
        }
            ...
            if let Some(cores) = self.clusters.get(&cluster) {
                unsafe {
                    ...
                    if result == 0 {
                        ...
                        return Err(format!("Error en sched_setaffinity: {}", std::io::Error::last_os_error()));
                    }
                }
            }
