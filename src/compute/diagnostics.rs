...
pub fn report_sparsity(total: u64, skipped: u64) {
    GLOBAL_DIAGNOSTICS.total_calculations.fetch_add(total, Ordering::Relaxed);
    GLOBAL_DIAGNOSTICS.skipped_calculations.fetch_add(skipped, Ordering::Relaxed);
}

/// Obtiene el porcentaje de Sparsity (Escasez) actual.
pub fn get_sparsity_report() -> f32 {
    let total = GLOBAL_DIAGNOSTICS.total_calculations.load(Ordering::Relaxed);
    let skipped = GLOBAL_DIAGNOSTICS.skipped_calculations.load(Ordering::Relaxed);
...
pub fn reset_diagnostics() {
    GLOBAL_DIAGNOSTICS.total_calculations.store(0, Ordering::Relaxed);
    GLOBAL_DIAGNOSTICS.skipped_calculations.store(0, Ordering::Relaxed);
}
