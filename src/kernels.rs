...
pub unsafe fn genomic_dot_product_neon(
    ...
    #[cfg(not(target_arch = "aarch64"))]
    {
        let mut sum = 0.0;
        for j in 0..n_blocks {
            let input_block = &input[j * stride * 4 ..];
            let weights_block = &weights[j * stride ..];
            let block_centroids = &centroids[j * 4 .. j * 4 + 4];
            
            for k in 0..stride {
                let byte = weights_block[k];
                for m in 0..4 {
                    let shift = (3 - m) * 2;
                    let bits = (byte >> shift) & 0b11;
                    let idx = (bits ^ (bits >> 1)) as usize;
                    sum += input_block[k * 4 + m] * block_centroids[idx];
                }
            }
        }
        sum
    ...
    #[cfg(target_arch = "aarch64")]
    {
        ...
        for i in 0..n_blocks {
            ...
            for j in 0..4 {
                ...
                if mode == 0 {
                    ...
                    d_v[j] = *lut_epi.get(dims * 16 + (b_idx << 2 | e_idx)).unwrap_or(&0.0);
                } else {
                    let eb = (*epi_strand.get(i).unwrap_or(&0) >> shift) & 0b11;
                    let tb = (*tri_strand.get(i).unwrap_or(&0) >> shift) & 0b11;
                    let e_idx = (eb ^ (eb >> 1)) as usize;
                    let t_idx = (tb ^ (tb >> 1)) as usize;
                    d_v[j] = *lut_tri.get(dims * 64 + (b_idx << 4 | e_idx << 2 | t_idx)).unwrap_or(&0.0);
                }
                dims += 1;
            }
        ...
        while dims < n_dims {
            ...
            if mode == 0 {
                ...
                let eb = (*epi_strand.get(i).unwrap_or(&0) >> shift) & 0b11;
                let e_idx = (eb ^ (eb >> 1)) as usize;
                total += *lut_epi.get(dims * 16 + (b_idx << 2 | e_idx)).unwrap_or(&0.0);
            } else {
                let eb = (*epi_strand.get(i).unwrap_or(&0) >> shift) & 0b11;
                let tb = (*tri_strand.get(i).unwrap_or(&0) >> shift) & 0b11;
                let e_idx = (eb ^ (eb >> 1)) as usize;
                let t_idx = (tb ^ (tb >> 1)) as usize;
                total += *lut_tri.get(dims * 64 + (b_idx << 4 | e_idx << 2 | t_idx)).unwrap_or(&0.0);
            }
            dims += 1;
        }
        return total.sqrt();
