...
fn main() {
        ...
        for epoch in 1..=1000 {
            ...
            if epoch % 50 == 0 {
                ...
                for _ in 0..500 {
                    ...
                    if mcts.nodes[node_idx].n_visits > 0 { mcts.expand(node_idx, 4, 0.05); }
                    ...
                    for i in 0..3 {
                        ...
                        if diff < 0.2 { score -= 1.0; } else { score += diff; }
                    }
                    mcts.backpropagate(last_idx, score);
                }
                let mut best_idx = 0; let mut max_q = -1000.0;
                for (idx, node) in mcts.nodes.iter().enumerate() {
                    if node.q_value > max_q && node.n_visits > 0 { max_q = node.q_value; best_idx = idx; }
                }
            ...
            if accuracy >= 90.0 { consecutive_high_acc += 1; } else { consecutive_high_acc = 0; }
    ...
    println!("\n✅ Bucle Caretaker finalizado en {:?}", start_time.elapsed());
}
