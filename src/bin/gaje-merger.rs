...
fn main() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    if args.len() < 4 {
        println!("Usage: gaje-merger <output.gaje> <model1.gaje> <model2.gaje> [<model3.gaje> ...]");
        return Ok(());
    }
    ...
    save_genomic_model(output_path, &merged, &first_config.unwrap(), first_tokenizer.as_ref())?;
