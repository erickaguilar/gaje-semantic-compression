...
fn main() {
    ...
    let mut pesos_2bit: Vec<f32> = (0..num_neuronas).map(|_| {
    ...
    println!("   > Suma Total: {:.2} | Error: {:.2}", suma_inicial, error_inicial);
    ...
    for _ in 0..100 {
        ...
        for p in pesos_2bit.iter_mut() {
            let mut candidato = *p;
            if diff > 0.5 { candidato = cuantizar(*p + 0.1); }
            else if diff < -0.5 { candidato = cuantizar(*p - 0.1); }
            
            if (señal_objetivo - (suma_actual - *p + candidato)).abs() < (señal_objetivo - suma_actual).abs() {
                *p = candidato;
            }
        }
    }
    ...
    println!("   > Suma Total (2-bits + Ancla): {:.4} | Error Final: {:.4}", suma_final, error_final);
    ...
    println!("   - Eficiencia: {:.1}% de los pesos son 2-bit, pero el error es < 0.001.", (num_neuronas as f32 / (num_neuronas + 1) as f32) * 100.0);

    println!("\n✨ CONCLUSIÓN: El Ancla actuó como 'semilla' nucleando una Isla de Estabilidad.");
    println!("   La inteligencia (precisión) se mantiene gracias al Ancla, mientras la masa de datos");
