...
///
/// # Safety
/// Esta función dereferencia un puntero raw `model_path`. El llamador debe asegurar que sea válido.
#[no_mangle]
pub unsafe extern "C" fn gaje_session_load(
...
///
/// # Safety
/// Esta función dereferencia punteros raw `session` y `user_input`.
#[no_mangle]
pub unsafe extern "C" fn gaje_session_chat(
    ...
    if session.is_null() || user_input.is_null() {
        return std::ptr::null_mut();
    }
    let session = &mut *session;
    let c_str = CStr::from_ptr(user_input);
...
///
/// # Safety
/// Esta función reconstruye un Box desde un puntero raw. El puntero debe haber sido obtenido vía `gaje_session_load`.
#[no_mangle]
pub unsafe extern "C" fn gaje_session_free(session: *mut GajeSession) {
    if !session.is_null() {
...
///
/// # Safety
/// Esta función reconstruye un CString desde un puntero raw para liberarlo.
#[no_mangle]
pub unsafe extern "C" fn gaje_string_free(s: *mut c_char) {
    if !s.is_null() {
