...
impl GajeIndex {
    ...
    pub fn calculate_distance_lut(&self, lut_base: &[f32], lut_epi: &[f32], lut_tri: &[f32], target_idx: usize) -> f32 {
        let strand_start = target_idx * self.stride;
        let strand_end = strand_start + self.stride;
        if strand_end > self.database.len() { return f32::MAX; }
        let strand = &self.database[strand_start..strand_end];
        let n_dims = lut_base.len() / 4;
        let has_mask = !self.precision_mask.is_empty();
        let has_epi_data = !self.epigenetic_database.is_empty() && self.epigenetic_database.len() >= strand_end;
        let has_tri_data = !self.triplet_database.is_empty() && self.triplet_database.len() >= strand_end;
        let default_mode = if has_tri_data { 2u8 } else if has_epi_data { 1u8 } else { 0u8 };
        #[cfg(target_arch = "aarch64")]
        {
            let epi_strand_actual = if has_epi_data { &self.epigenetic_database[strand_start..strand_end] } else { &[] };
            let tri_strand_actual = if has_tri_data { &self.triplet_database[strand_start..strand_end] } else { &[] };
            let dummy_mask;
            let mask = if has_mask { &self.precision_mask } else { dummy_mask = vec![default_mode; self.stride]; &dummy_mask };
            let dummy_e;
            let e_s = if epi_strand_actual.is_empty() { dummy_e = vec![0u8; self.stride]; &dummy_e } else { epi_strand_actual };
            let dummy_t;
        ...
        for i in 0..self.stride {
            let mut mode = if has_mask { self.precision_mask[i] } else { default_mode };
            if mode == 2 && !has_tri_data { mode = if has_epi_data { 1 } else { 0 }; }
            if mode == 1 && !has_epi_data { mode = 0; }
            let byte = strand[i];
            let offset = strand_start + i;
            for j in 0..4 {
                if dims >= n_dims { break; }
                let shift = (3 - j) * 2;
                let bb = (byte >> shift) & 0b11;
                let b_idx = (bb ^ (bb >> 1)) as usize;
                if mode == 0 { dist_sq += lut_base[dims * 4 + b_idx]; }
                else if mode == 1 {
    ...
    pub fn search_layer_lut(&self, lut_base: &[f32], lut_epi: &[f32], lut_tri: &[f32], ep: usize, ef: usize, level: usize) -> BinaryHeap<Neighbor> {
    ...
    fn insert_node(&mut self, idx: usize) {
        ...
        if self.entry_point.is_none() { self.max_level = level; self.entry_point = Some(idx); self.layers = vec![vec![vec![]; n]; (level + 1) as usize]; return; }
        
        let strand_start = idx * self.stride;
        let q = dequantize_embedding(self.database[strand_start..strand_start+self.stride].to_vec(), self.stride * 4, Some(self.centroids.clone())).unwrap();
        let mut lut_base = Vec::with_capacity(q.len() * 4);
        let c = &self.centroids;
        let is_multi = c.len() == q.len() * 4;
        for (d_idx, &val) in q.iter().enumerate() { for b in 0..4 { let diff = val - (if is_multi { c[d_idx*4 + b] } else { c[b] }); lut_base.push(diff * diff); } }
        ...
        if !self.triplet_centroids.is_empty() {
            ...
            for (d_idx, &val) in q.iter().enumerate() {
                let cb = if is_multi { &self.centroids[d_idx*4..(d_idx+1)*4] } else { &self.centroids[0..4] };
                let ce = if !self.epigenetic_centroids.is_empty() { if self.epigenetic_centroids.len() == q.len() * 4 { &self.epigenetic_centroids[d_idx*4..(d_idx+1)*4] } else { &self.epigenetic_centroids[0..4] } } else { &[0.0; 4] };
                let ct = if c_tri.len() == q.len() * 4 { &c_tri[d_idx*4..(d_idx+1)*4] } else { &c_tri[0..4] };
                for &b_val in cb { for &e_val in ce { for &t_val in ct { let diff = val - (b_val + e_val + t_val); lut_tri.push(diff * diff); } } }
            }
        }
        let mut curr_ep = self.entry_point.unwrap();
        for l in (level + 1..=self.max_level).rev() { let res = self.search_layer_lut(&lut_base, &lut_epi, &lut_tri, curr_ep, 1, l as usize); if let Some(best) = res.peek() { curr_ep = best.idx; } }
        for l in (0..=std::cmp::min(level, self.max_level)).rev() {
            let neighbors = self.search_layer_lut(&lut_base, &lut_epi, &lut_tri, curr_ep, self.ef_construction, l as usize);
            ...
            for n in neighbors_vec.iter().take(m_limit) { if !self.layers[l as usize][idx].contains(&n.idx) { self.layers[l as usize][idx].push(n.idx); } if !self.layers[l as usize][n.idx].contains(&idx) { self.layers[l as usize][n.idx].push(idx); } }
            if let Some(best) = neighbors_vec.first() { curr_ep = best.idx; }
        }
        if level > self.max_level { for _ in self.max_level..level { self.layers.push(vec![vec![]; n]); } self.max_level = level; self.entry_point = Some(idx); }
    }
}
#[pymethods]
impl GajeIndex {
    ...
    pub fn new(database: Vec<Vec<u8>>, centroids: Vec<f32>, epigenetic_database: Vec<Vec<u8>>, epigenetic_centroids: Vec<f32>, triplet_database: Vec<Vec<u8>>, triplet_centroids: Vec<f32>, precision_mask: Vec<u8>, m: usize, ef_construction: usize) -> Self {
        ...
        GajeIndex { database: flat_db, centroids, epigenetic_database: flat_epi_db, epigenetic_centroids, triplet_database: flat_tri_db, triplet_centroids, precision_mask, stride, layers: Vec::new(), max_level: -1, entry_point: None, ef_construction, m, level_mult: 1.0 / (m as f64).ln() }
    }
    pub fn build(&mut self) -> PyResult<()> {
        ...
        for i in 0..n { self.insert_node(i); }
        Ok(())
    }
        ...
        let mut results: Vec<(usize, f32)> = (0..n).into_par_iter().map(|idx| {
            ...
            for &byte in strand {
                for j in 0..4 {
                    ...
                    let centroid = if is_multi { c[dims * 4 + c_idx] } else { c[c_idx] };
    ...
    pub fn search(&self, query_vector: Vec<f32>, k: usize, ef: Option<usize>) -> PyResult<Vec<(usize, f32)>> {
        ...
        for (d_idx, &val) in query_vector.iter().enumerate() { let cd = if is_multi { &c_base[d_idx*4..(d_idx+1)*4] } else { &c_base[0..4] }; for b in 0..4 { let diff = val - cd[b]; lut_base.push(diff * diff); } }
        let mut lut_epi = Vec::new();
        if !self.epigenetic_centroids.is_empty() {
        ...
        if !self.triplet_centroids.is_empty() {
            ...
            for (d_idx, &val) in query_vector.iter().enumerate() {
                let cb = if is_multi { &self.centroids[d_idx*4..(d_idx+1)*4] } else { &self.centroids[0..4] };
                let ce = if !self.epigenetic_centroids.is_empty() { if self.epigenetic_centroids.len() == query_vector.len() * 4 { &self.epigenetic_centroids[d_idx*4..(d_idx+1)*4] } else { &self.epigenetic_centroids[0..4] } } else { &[0.0; 4] };
                let ct = if c_tri.len() == query_vector.len() * 4 { &c_tri[d_idx*4..(d_idx+1)*4] } else { &c_tri[0..4] };
                for &b_val in cb { for &e_val in ce { for &t_val in ct { let diff = val - (b_val + e_val + t_val); lut_tri.push(diff * diff); } } }
            }
        }
        let mut curr_ep = self.entry_point.unwrap();
        for l in (1..=self.max_level).rev() { let res = self.search_layer_lut(&lut_base, &lut_epi, &lut_tri, curr_ep, 1, l as usize); if let Some(best) = res.peek() { curr_ep = best.idx; } }
        let final_neighbors = self.search_layer_lut(&lut_base, &lut_epi, &lut_tri, curr_ep, ef_val, 0);
