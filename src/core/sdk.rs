...
impl GajeSession {
    ...
    pub fn generate(&mut self, prompt: &str, max_tokens: usize, temperature: f32, top_p: f32) -> Result<String, Box<dyn Error>> {
        ...
        for _ in 0..max_tokens {
            let next_id = self.sampler.sample_core(current_logits.clone(), temperature, top_p)
...
mod tests {
    ...
    fn test_sdk_session_lifecycle() {
        ...
        if fs::metadata(tokenizer_json).is_ok() {
            let tokenizer = GajeTokenizer::from_file(tokenizer_json).expect("Error cargando tokenizer");
        ...
        let model = init_born_genomic_model(model_path, config.clone(), vocab_size).expect("Error inicializando modelo");
