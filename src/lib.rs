...
/// Cuantización Genómica de 2 bits con soporte para múltiples umbrales (por dimensión o globales)
#[pyfunction]
...
pub fn dna_similarity_search_adc(query_vector: Vec<f32>, database: Vec<Vec<u8>>, centroids: Option<Vec<f32>>) -> PyResult<Vec<(usize, f32)>> {
    ...
    for (idx, strand) in database.iter().enumerate() {
        ...
        results.push((idx, squared_distance.sqrt()));
    }
