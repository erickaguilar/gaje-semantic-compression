...
impl GGUFReader {
    pub fn open(path: &str) -> std::io::Result<Self> {
        ...
        for _ in 0..metadata_kv_count {
            ...
            let val_type = Self::read_u32(&mut file)?;
            let value = Self::read_value(&mut file, val_type)?;
        ...
        for _ in 0..tensor_count {
            ...
            for _ in 0..n_dims {
                shape.push(Self::read_u64(&mut file)?);
            }
            let tensor_type_id = Self::read_u32(&mut file)?;
    ...
    pub fn get_tensor_data(&mut self, name: &str) -> std::io::Result<Vec<u8>> {
    ...
    fn read_string(f: &mut File) -> std::io::Result<String> {
        let len = Self::read_u64(f)?;
        let mut buf = vec![0u8; len as usize];
        f.read_exact(&mut buf)?;
        String::from_utf8(buf).map_err(|e| std::io::Error::new(std::io::ErrorKind::InvalidData, e))
    }

    fn read_value(f: &mut File, val_type: u32) -> std::io::Result<GGUFValue> {
        match val_type {
            0 => { // Uint8
                let mut buf = [0u8; 1];
                f.read_exact(&mut buf)?;
            ...
            4 => Ok(GGUFValue::Uint32(Self::read_u32(f)?)),
            5 => { // Int32
                let mut buf = [0u8; 4];
                f.read_exact(&mut buf)?;
            ...
            8 => Ok(GGUFValue::String(Self::read_string(f)?)),
            9 => { // Array
                let item_type = Self::read_u32(f)?;
                let len = Self::read_u64(f)?;
                let mut items = Vec::with_capacity(len as usize);
                for _ in 0..len {
                    items.push(Self::read_value(f, item_type)?);
                }
                Ok(GGUFValue::Array(items))
            }
            10 => Ok(GGUFValue::Uint64(Self::read_u64(f)?)),
            11 => Ok(GGUFValue::Int64(Self::read_u64(f)? as i64)),
            12 => { // Float64
                let mut buf = [0u8; 8];
                f.read_exact(&mut buf)?;
