...
pub fn save_smg1_model(path: &str, model: &Smg1Model, config: &Smg1Config) -> std::io::Result<()> {
    ...
    batch
        .write_metadata(
            "vocabulary",
            &serde_json::to_string(&model.id_to_word).unwrap(),
        )
        .unwrap();
    ...
    for (i, layer) in model.layers.iter().enumerate() {
        ...
        batch
            .write_tensor(&format!("{}.weights", p), &compress(&layer.packed_weights))
            .unwrap();
        batch
            .write_tensor(
                &format!("{}.potentials_real", p),
                &compress(f32_u8(&layer.membrane_potentials_real)),
            )
            .unwrap();
        batch
            .write_tensor(
                &format!("{}.potentials_imag", p),
                &compress(f32_u8(&layer.membrane_potentials_imag)),
            )
            .unwrap();
        batch
            .write_tensor(
                &format!("{}.thresholds", p),
                &compress(f32_u8(&layer.thresholds)),
            )
            .unwrap();
        batch
            .write_tensor(&format!("{}.decays", p), &compress(f32_u8(&layer.decays)))
            .unwrap();
        batch
            .write_tensor(
                &format!("{}.anchors", p),
                &compress(&layer.anchors_sparse_buffer()),
            )
            .unwrap();
    }
...
pub fn load_smg1_model(path: &str) -> std::io::Result<(Smg1Model, Smg1Config)> {
    ...
    let id_to_word: Vec<String> = {
        let table = read_txn
            .open_table(METADATA_TABLE)
            .map_err(|e| std::io::Error::other(e.to_string()))?;
        let vocab_json = table
            .get("vocabulary")
            .map_err(|e| std::io::Error::other(e.to_string()))?
            .ok_or_else(|| {
                std::io::Error::new(std::io::ErrorKind::NotFound, "vocabulary not found")
            })?
            .value()
            .to_string();
    ...
    for i in 0..config.layer_dims.len() {
        ...
        let mut membrane_potentials_real =
            get_tensor_f32(&read_txn, &format!("{}.potentials_real", p));
        let mut membrane_potentials_imag =
            get_tensor_f32(&read_txn, &format!("{}.potentials_imag", p));
