...
class GenomicLLM:
    def __init__(self, model_path):
        ...
        self.token_embd = dequantize_q8_0(
            next(t for t in self.reader.tensors if t.name == "token_embd.weight")
        )
...
class TransformerBlock:
    def __init__(self, reader, idx, n_head, n_head_kv, head_dim, rope_base, eps):
        ...
        self.w_gate = dequantize_q8_0(
            next(t for t in reader.tensors if t.name == p + "ffn_gate.weight")
        )
        self.w_up = dequantize_q8_0(
            next(t for t in reader.tensors if t.name == p + "ffn_up.weight")
        )
        self.w_down = dequantize_q8_0(
            next(t for t in reader.tensors if t.name == p + "ffn_down.weight")
        )
...
class AttentionLayer:
    def __init__(self, reader, p, n_head, n_head_kv, head_dim, rope_base):
        ...
        self.k_cache = []
        self.v_cache = []
        self.w_q = dequantize_q8_0(
            next(t for t in reader.tensors if t.name == p + "attn_q.weight"),
            n_head,
            head_dim,
            True,
        )
        self.w_k = dequantize_q8_0(
            next(t for t in reader.tensors if t.name == p + "attn_k.weight"),
            n_head_kv,
            head_dim,
            True,
        )
        self.w_v = dequantize_q8_0(
            next(t for t in reader.tensors if t.name == p + "attn_v.weight")
        )
        self.w_o = dequantize_q8_0(
            next(t for t in reader.tensors if t.name == p + "attn_output.weight")
        )
    ...
    def apply_rope(self, x, pos):
        ...
        res = np.zeros_like(x)
        inv_freq = 1.0 / (
            self.rope_base ** (np.arange(0, dim, 2, dtype=np.float32) / dim)
        )
        theta = pos * inv_freq
        cos = np.cos(theta)
        sin = np.sin(theta)
    ...
    def forward(self, x, pos):
        ...
        k_full = np.stack(self.k_cache)
        v_full = np.stack(self.v_cache)
