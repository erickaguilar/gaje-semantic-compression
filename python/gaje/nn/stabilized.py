...
class GenomicLayer:
    def __init__(
        ...
        if hasattr(weights_f32_or_tensor, "tensor_type"):
            ...
            self.out_features, self.in_features = (
                tensor.shape[::-1] if len(tensor.shape) == 2 else (tensor.shape[0], 1)
            )
            ...
            if tensor.tensor_type == gguf.GGMLQuantizationType.F16:
                ...
                dna, centroids, anchors = dna_semantic_compression.genomize_f16_native(
                    tensor.data.tobytes(), block_size, a_threshold
                )
    ...
    def _init_from_f32(self, weights_f32, block_size, anchor_threshold):
        ...
        for i in range(len(reshaped)):
            ...
            anchors_f32[row_idx, col_in_row : col_in_row + block_size][
                anchor_mask
            ] = residual[anchor_mask]
    ...
    def get_row(self, idx):
        ...
        anchor_row = np.array(
            self.linear.anchors[idx * self.in_features : (idx + 1) * self.in_features]
        )
...
class GenomicTransformerBlock:
    def __init__(self, reader, idx, n_head, n_head_kv, head_dim, rope_base, eps):
        ...
        p = f"blk.{idx}."
        attn_norm = next(
            t for t in reader.tensors if t.name == p + "attn_norm.weight"
        ).data.astype(np.float32)
        ffn_norm = next(
            t for t in reader.tensors if t.name == p + "ffn_norm.weight"
        ).data.astype(np.float32)
...
class GenomicLLM:
    def __init__(self, model_path, num_blocks=None):
        print(
            f"🧬 Sincronizando Organismo Genómico (2-bit): {os.path.basename(model_path)}"
        )
        ...
        self.n_head = int(
            self.reader.fields[f"{arch}.attention.head_count"].parts[-1][0]
        )
        self.n_head_kv = int(
            self.reader.fields[f"{arch}.attention.head_count_kv"].parts[-1][0]
        )
        self.head_dim = self.n_embd // self.n_head
        self.n_blocks = (
            int(self.reader.fields[f"{arch}.block_count"].parts[-1][0])
            if num_blocks is None
            else num_blocks
        )
        self.rope_base = float(
            self.reader.fields[f"{arch}.rope.freq_base"].parts[-1][0]
        )
        self.eps = float(
            self.reader.fields[f"{arch}.attention.layer_norm_rms_epsilon"].parts[-1][0]
        )
        ...
        self.embeddings = GenomicLayer(
            "token_embd", w_embd_tensor, balancer=self.balancer
        )

        self.output_norm = next(
            t for t in self.reader.tensors if t.name == "output_norm.weight"
        ).data.astype(np.float32)

        try:
            w_head_tensor = next(
                t for t in self.reader.tensors if t.name == "output.weight"
            )
        ...
        for i in range(self.n_blocks):
            ...
            if (i + 1) % 5 == 0:
                print(f"    [~] Bloque {i+1}/{self.n_blocks} genomizado...")
    ...
    def rms_norm(self, x, weight):
        ...
        if x.shape != weight.shape:
            print(
                f"DEBUG SHAPE: x={x.shape}, weight={weight.shape}, name={self.__class__.__name__}"
            )
        return (x / rms) * weight
    ...
    def forward(self, tokens, use_cache=False, start_pos=0):
        if not use_cache:
            self.clear_cache()
        all_logits = []
    ...
    def generate(self, prompt, max_new_tokens=25):
        print(f"\n📝 Prompt: '{prompt}'")
        print("[*] Generando", end="", flush=True)

        # Cargar el tokenizer correcto (el mismo que usa el modelo base Qwen2)
        if "SmolLM" in self.tokenizer.name_or_path:
        ...
        # Pre-fill context
        logits = self.forward(token_ids, use_cache=True, start_pos=0)
        ...
        for i in range(1, max_new_tokens):
            pos = len(token_ids) - 1
            logits = self.forward([next_token], use_cache=True, start_pos=pos)
            next_token = np.argmax(logits[-1])
            token_ids.append(next_token)
