...
class GenomicTrainer:
    def __init__(self, model, lr=0.01, use_torch=True, resonance_weight=0.05):
        ...
        if self.use_torch:
            print(f"[*] GenomicTrainer: Usando PyTorch (Resonance Weight: {resonance_weight})")
        else:
            print(f"[*] GenomicTrainer: Usando NumPy (Resonance Weight: {resonance_weight})")
    ...
    def train_step(self, input_ids, target_ids, phase=1):
        ...
        if phase >= 2:
            ...
            self.model.rust_llm.train_on_sequence(input_ids + [target_ids[-1]], self.lr * 0.5)
    ...
    def fit(self, dataset, epochs=10):
        ...
        for epoch in range(epochs):
            ...
            if count > 0:
                ...
                print(f"    - Época {epoch+1}/{epochs} [{phase_name}] | Loss: {avg_loss:.4f} | PPL: {ppl:.2f} | {duration:.2f}s")
            else:
