        ...
        for t_idx, teacher in enumerate(self.teachers):
            ...
            for i, tid in enumerate(input_ids):
                ...
                for b_idx, block in enumerate(teacher.blocks):
                    ...
                    council_stats[f"{prefix}input"] += np.abs(x_arr) / len(self.teachers)
                    
                    # Forward
                    x = block.forward(x, pos=i)
                    
        for name in council_stats: council_stats[name] /= len(input_ids)
        return council_stats

    def run_distillation_pipeline(self, prompts, output_dir="gaje_council_model"):
        print(f"🚀 Iniciando Pipeline de Destilación por Consejo ({len(self.teachers)} Maestros).")
        ...
        for i in tqdm(range(self.num_blocks), desc="Destilando Bloques"):
            ...
            for teacher_reader in [gguf.GGUFReader(p) for p in self.model_paths]:
                w = dequantize_q8_0(next(t for t in teacher_reader.tensors if t.name == prefix + "attn_q.weight"))
                if w_q_agg is None: w_q_agg = np.zeros_like(w)
            ...
            block_student.attn.attn.centroids = self.calibrate_layer_with_activations(w_q_agg, get_thresholds(w_q_agg), stats_in)
