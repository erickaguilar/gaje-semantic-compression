...
def dequantize_q8_0(tensor, n_head=None, head_dim=None, is_q_or_k=False, rope_style="split"):
    ...
    try:
        ...
        res = dna_semantic_compression.dequantize_q8_0_native(data_u8.tobytes(), out_f, in_f)
        ...
        for b in range(n_blocks):
            offset = b * 34
            delta = np.frombuffer(data_u8[offset:offset+2], dtype=np.float16)[0].astype(np.float32)
