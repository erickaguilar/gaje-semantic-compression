...
class GenomicTransformerBlock:
        ...
        for tensor in reader.tensors:
            if tensor.name.startswith(prefix) and "weight" in tensor.name:
                if len(tensor.shape) == 2:
                    ...
                    self.layers[short_name] = GenomicLayer(
                        tensor.name, tensor.data.astype(np.float32)
                    )
    ...
    def forward(self, x, pos=0):
        ...
        if "attn_output" in self.layers:
            attn_out = self.layers["attn_output"].forward(v_expanded.tolist())[
                : len(x_in)
            ]
        else:
...
class GenomicLLM:
    def __init__(self, model_path, num_blocks=2):
        ...
        try:
            ...
            if w_head.shape[1] != self.hidden_dim:
                w_head = w_head.T
    ...
    def generate_step(self, text, pos=0, silent=False):
        ...
        x = np.array(
            dna_semantic_compression.dequantize_embedding(
                list(dna_strand), self.hidden_dim, self.embeddings.centroids
            )
        )
        ...
        if not silent:
            print(
                f"[*] Procesando ADN a través de {len(self.blocks)} bloques genómicos con RoPE (pos={pos})..."
            )
...
def run_full_pipeline():
    ...
    total_dna_bytes = (
        ...
        + sum(
            [
                sum([len(ly.engine.database) for ly in bl.layers.values()])
                for bl in llm.blocks
            ]
        )
