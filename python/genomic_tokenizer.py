...
class GenomicTokenizer:
    ...
    def encode(self, text):
        ...
        for tid in token_ids:
            ...
            dna_strand = self.engine.database[tid * self.engine.stride : (tid + 1) * self.engine.stride]
...
def test_genomic_tokenizer():
    ...
    if not os.path.exists(model_path):
        print("❌ Error: No se encontró el modelo Qwen2 para extraer los pesos de embedding.")
        return
    ...
    print(f"   • ADN Semántico: {len(dna_seq)} strands de {len(dna_seq[0])} bytes cada uno.")
    print(f"   • RAM Total ADN: {(len(dna_seq) * len(dna_seq[0]))} bytes")
    print(f"   • RAM F32 Equiv: {(len(dna_seq) * weights_f32.shape[1] * 4)} bytes (16x ahorro)")
    ...
    rec_signal = dna_semantic_compression.dequantize_embedding(list(first_token_dna), weights_f32.shape[1], g_tokenizer.centroids)
    
    # Comparar con el peso original
    orig_signal = weights_f32[ids[0]]
    cos_sim = np.dot(orig_signal, rec_signal) / (np.linalg.norm(orig_signal) * np.linalg.norm(rec_signal))
    
    print(f"   • Similitud Coseno (Token '{g_tokenizer.decode([ids[0]])}'): {cos_sim:.4f}")
