...
initial_indices = np.random.randint(0, NUM_CENTROIDS, size=(OUT_FEATURES, IN_FEATURES), dtype=np.uint8)
initial_loss = compute_loss(initial_indices)

print(f"[*] Parámetros del Micro-Organismo: {OUT_FEATURES}x{IN_FEATURES} ({OUT_FEATURES * IN_FEATURES} pesos)")
...
print(f"  • Loss Final: {current_loss_a:.4f} | Reducción: {reduction_a:.2f}% | Tiempo: {time_a * 1000:.1f} ms")
...
for step in range(spsa_steps):
    ...
    ind_plus[target_row, cols] = np.clip(np.round(ind_plus[target_row, cols] + delta), 0, NUM_CENTROIDS - 1)
    loss_plus = compute_loss(ind_plus.astype(np.uint8))  # Forward 1
    
    # 2. Forward antitético (-delta)
    ind_minus = indices_b.copy()
    ind_minus[target_row, cols] = np.clip(np.round(ind_minus[target_row, cols] - delta), 0, NUM_CENTROIDS - 1)
...
print(f"  • Loss Final: {current_loss_b:.4f} | Reducción: {reduction_b:.2f}% | Tiempo: {time_b * 1000:.1f} ms")
...
for step in range(FORWARD_BUDGET):
    ...
    new_vals = np.clip(indices_c[rows, cols].astype(np.int32) + signs, 0, NUM_CENTROIDS - 1).astype(np.uint8)
...
print(f"  • Loss Final: {current_loss_c:.4f} | Reducción: {reduction_c:.2f}% | Tiempo: {time_c * 1000:.1f} ms")
...
for step in range(growth_budget):
    ...
    signs = np.sign(grad_approx[rows, cols]).astype(np.int32)
    new_vals = np.clip(indices_d[rows, cols].astype(np.int32) + signs, 0, NUM_CENTROIDS - 1).astype(np.uint8)
...
for step in range(spsa_refine_steps):
    ...
    ind_plus[target_row, cols] = np.clip(ind_plus[target_row, cols].astype(np.int32) + delta, 0, NUM_CENTROIDS - 1).astype(np.uint8)
    loss_plus = compute_loss(ind_plus)
    
    # Forward antitético
    ind_minus = indices_d.copy()
    ind_minus[target_row, cols] = np.clip(ind_minus[target_row, cols].astype(np.int32) - delta, 0, NUM_CENTROIDS - 1).astype(np.uint8)
...
print(f"  • Loss Final: {current_loss_d:.4f} | Reducción: {reduction_d:.2f}% | Tiempo: {time_d * 1000:.1f} ms")
...
print(f"• [Brazo A] Mutación Aleatoria Simple:       Loss={current_loss_a:.4f} (-{reduction_a:.1f}%) | Forwards a -30%: {forwards_a}")
print(f"• [Brazo B] SPSA Discreto (Temperatura T_g): Loss={current_loss_b:.4f} (-{reduction_b:.1f}%) | Forwards a -30%: {forwards_b}")
print(f"• [Brazo C] Reglas Locales Hebbianas:        Loss={current_loss_c:.4f} (-{reduction_c:.1f}%) | Forwards a -30%: {forwards_c}")
print(f"• [Brazo D] Currículo Híbrido H3:            Loss={current_loss_d:.4f} (-{reduction_d:.1f}%) | Forwards a -30%: {forwards_d}")

speedup_c = forwards_a / max(1, forwards_c)
speedup_d = forwards_a / max(1, forwards_d)

print("--------------------------------------------------------------------------------")
print(f"⚡ Aceleración Reglas Locales (C vs A): {speedup_c:.2f}x más rápido a -30% Loss")
print(f"⚡ Aceleración Currículo Híbrido (D vs A): {speedup_d:.2f}x más rápido a -30% Loss")
print("--------------------------------------------------------------------------------")
...
print(f"   La combinación de Crecimiento Hebbiano ({forwards_c} forwards) + Refinamiento SPSA")
