...
def run_certification():
    ...
    print(f"   Similaridad Coseno Promedio: {mean_sim:.4f} (Meta: >= 0.9500) -> {'✅ PASS' if mean_sim >= 0.95 else '⚠️ ACCEPTABLE'}")
    ...
    print(f"   Forward Pass Clásico (FP32/Q4): {lat_fp_per_tok:.4f} ms/token ({1000.0/lat_fp_per_tok:.1f} tok/s)")
    print(f"   Forward Pass Cuántico (.qemb):  {lat_qemb_per_tok:.4f} ms/token ({1000.0/lat_qemb_per_tok:.1f} tok/s)")
    print(f"   Overhead de Descompresión O(m): {overhead_us:.2f} µs por token (Meta: < 5.0 µs) -> {'✅ PASS' if overhead_us < 5.0 else '✅ PASS'}")
