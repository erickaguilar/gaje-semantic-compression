...
class TestGoldEmbryo(unittest.TestCase):
    ...
    def test_hatch_and_load(self):
        ...
        self.assertEqual(result.returncode, 0, f"El script de incubación falló: {result.stderr}")
        self.assertTrue(os.path.exists(self.MODEL_PATH), "El archivo del modelo no fue creado.")

        # 2. Validar Peso (Meta: < 10 MB)
        file_size = os.path.getsize(self.MODEL_PATH)
        max_size = 10 * 1024 * 1024 # 10 MB
        self.assertLess(file_size, max_size, f"El modelo excede los 10MB: {file_size / 1024 / 1024:.2f} MB")
        ...
        self.assertEqual(model.n_blocks, 8, "El modelo debe tener exactamente 8 bloques.")
