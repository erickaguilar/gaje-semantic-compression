...
mod tests {
    ...
    fn test_phase_survival_toroidal_echo() {
        ...
        let mut model = match loader_res {
            ...
            Err(e) => {
                println!(
                    "⚠️ No se pudo inicializar NativeLoader en {}: {}",
                    model_db_path, e
                );
                return;
            }
        ...
        println!(
            "⏳ Inyectando señal (Token ID: {}) en el Toroide...",
            needle_token_id
        );
        // Desglose del forward para detectar NaNs paso a paso
        let mut h = model
            .embeddings
            .get_row_core(needle_token_id)
            .expect("Error en embeddings");
        ...
        for (i, block) in model.blocks.iter_mut().enumerate() {
            h = block
                .forward_core(h, 0)
                .expect(&format!("Error en bloque {}", i));
            if h.iter().any(|x| x.is_nan()) {
        ...
        let h_norm =
            unsafe { _impl::compute::kernels::rms_norm(&h, &model.output_norm, model.eps) };
        ...
        if gold_standard_hidden.iter().any(|x| x.is_nan()) {
            println!(
                "🚨 ERROR CRÍTICO: El Gold Standard ya contiene NaNs antes de iniciar la deriva."
            );
            return;
        } else {
        ...
        for i in 0..cycles {
            ...
            if i % 1_000 == 0 && i > 0 {
                ...
                if has_nan {
                    println!(
                        "🚨 COLAPSO NUMÉRICO DETECTADO en el ciclo {}. Abortando para diagnóstico.",
                        i
                    );
        ...
        println!(
            "✨ Deriva completada en {:?}. Procediendo al Colapso de Fase...",
            duration
        );

        let (_, final_hidden) = model
            .forward_with_hidden_core(needle_token_id, false)
        ...
        if similarity >= threshold {
            ...
            assert!(
                ...
                "La señal se disipó en el toroide. Similitud: {}",
                similarity
            );
        }
        println!("========================================");
