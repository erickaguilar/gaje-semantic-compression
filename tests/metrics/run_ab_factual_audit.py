...
def main():
    ...
    for name, path in models_to_test:
        ...
        if proc.returncode == 0 and os.path.exists(temp_file):
            ...
            print(f"✅ Process finished in {duration:.2f}s. Loaded in {res_dict['load_time_sec']:.2f}s.")
            for r in res_dict['results']:
                # Print only first line of response or brief snippet
                snippet = r['response'].split('\n')[0][:80]
                print(f"  - Prompt: '{r['prompt']}'")
                print(f"    Response: {snippet!r} (generated {r['tokens_generated']} tokens at {r['tokens_per_sec']:.2f} tok/s)")
