...
def main():
    ...
    print(f"  - Delta PPL (<1%):    {delta_ppl:.2f}% {'✅' if delta_ppl < 1.0 else '❌'}")
    print(f"  - Recuperación DNI:   {'✅ PASSED' if success else '❌ FAILED'}")
    
    cert_passed = delta_ppl < 1.0 and success
    print(f"  - ESTADO FINAL:       {'✅ CERTIFICABLE' if cert_passed else '❌ NO CERTIFICABLE'}")
    print("="*60)

    # Limpieza
    if os.path.exists(needle_file): os.remove(needle_file)
