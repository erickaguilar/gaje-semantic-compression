...
def test_recall_at_10_adc():
    ...
    db_vecs = np.array([normalize(v) for v in np.dot(latent, projection).astype(np.float32)])
    
    # Entrenar codebook y cuantizar
    codebook = train_genomic_codebook(db_vecs, mode="per_dim")
    db_dna = [dna_core.quantize_embedding(v.tolist(), codebook["thresholds"]) for v in db_vecs]
    ...
    for _ in range(20):
        ...
        adc_results = dna_core.dna_similarity_search_adc(query_vec.tolist(), db_dna, codebook["centroids"])
...
def test_next_token_prediction_precision():
    ...
    assert "France" in predicted_token or "France" in llm.tokenizer.decode([top_id]), f"Predicción inesperada: {predicted_token}"
