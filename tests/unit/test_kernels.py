...
def test_mixed_precision_kernel_phase12():
    ...
    weights_base = np.random.normal(0, 0.1, (out_features, in_features)).astype(
        np.float32
    )
    thresholds = [-0.1, 0.0, 0.1]
    centroids = [-0.15, -0.05, 0.05, 0.15]
    dna_base = b"".join(
        [dna_core.quantize_embedding(row.tolist(), thresholds) for row in weights_base]
    )
    ...
    epi_strands = b"".join(
        [
            dna_core.quantize_embedding(
                np.random.normal(0, 0.05, 8).tolist(), thresholds
            )
    ...
    tri_strands = b"".join(
        [
            dna_core.quantize_embedding(
                np.random.normal(0, 0.02, 4).tolist(), thresholds
            )
            for _ in range(out_features)
        ]
    )
