...
def test_genomic_norm_evolution():
    ...
    assert any(h != 1.0 for h in new_h_scales), "h_scale should have changed after mutation"
    ...
    assert all(abs(h - 1.0) < 1e-6 for h in final_h_scales), "Rollback should restore h_scale to 1.0"
