...
def test_q8_0_quantization():
    ...
    np.random.seed(42)
    original_weights = np.random.uniform(-2.0, 2.0, size=(out_features, in_features)).astype(np.float32)
    ...
    reconstructed = np.array(
        dna_semantic_compression.dequantize_q8_0_native(layer.dna_database, out_features, in_features),
        dtype=np.float32
