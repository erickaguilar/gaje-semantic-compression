...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    unsafe {
        _impl::compute::kernels::init_shuffle_table();
    }
    ...
    let mut i = 1;
    while i < args.len() {
        if args[i] == "--student" && i + 1 < args.len() {
            ...
            epochs = args[i + 1].parse().unwrap_or(5);
            i += 2;
        } else if args[i] == "--lr" && i + 1 < args.len() {
            lr = args[i + 1].parse().unwrap_or(0.005);
            i += 2;
        } else {
            i += 1;
        }
    ...
    fn check_linear(l: &_impl::nn::linear::GenomicLinear) -> bool {
        l.centroids
            .iter()
            .any(|&x| x.is_nan() || x.is_infinite() || x.abs() > 1e10)
    }

    let mut corruption_found = check_linear(&student.embeddings) || check_linear(&student.lm_head);
    if !corruption_found {
        for block in &student.blocks {
            if check_linear(&block.q_gen)
                || check_linear(&block.k_gen)
                || check_linear(&block.v_gen)
                || check_linear(&block.w_o)
                || check_linear(&block.gate_gen)
                || check_linear(&block.up_gen)
    ...
    let bad_norm = student
        .output_norm
        .iter()
        .any(|&x| x.is_nan() || x.is_infinite() || x == 0.0)
        || student.output_norm.is_empty();

    if corruption_found || bad_norm {
        println!(
            "[!] LIMPIEZA PROFUNDA DE NaNs (Corrupción: {}, BadNorm: {}). Reseteando Estructura...",
            corruption_found, bad_norm
        );
    ...
    if final_nan {
        ...
        let mut reset_l = |l: &mut _impl::nn::linear::GenomicLinear, scale: f32| {
            ...
            for x in &mut l.epigenetic_centroids {
                if x.is_nan() || x.is_infinite() {
                    *x = 0.0;
                }
            }
            for x in &mut l.triplet_centroids {
                if x.is_nan() || x.is_infinite() {
                    *x = 0.0;
                }
            }
            ...
            for v in &mut a_vals {
                ...
                if f.is_nan() || f.is_infinite() {
                    *v = half::f16::from_f32(0.0);
                    a_changed = true;
                }
            }
            if a_changed {
                l.anchor_values = std::sync::Arc::new(a_vals);
            }
        };
        ...
        student.eps = 1e-4;
        for block in &mut student.blocks {
            reset_l(&mut block.q_gen, 0.01);
            reset_l(&mut block.k_gen, 0.01);
            reset_l(&mut block.v_gen, 0.01);
            reset_l(&mut block.w_o, 0.01);
            reset_l(&mut block.gate_gen, 0.01);
            reset_l(&mut block.up_gen, 0.01);
            ...
            block.eps = 1e-4;
            block.attn.eps = 1e-4;
            if block.h_scale.is_nan() || block.h_scale.is_infinite() {
                block.h_scale = 1.0;
            }
        ...
        println!(
            "    [✔] Modelo base generado con Ruptura de Simetría y Normas Limpias. Procediendo..."
        );
    }
    ...
    println!(
        "\n🚀 Iniciando Ciclo de Crianza Equilibrada ({} épocas)...",
        epochs
    );
    ...
    'training_loop: for epoch in 0..epochs {
            ...
            match distiller.distill_step(&mut student, &text, lr) {
                Ok(loss) => {
                    ...
                    if count % 100 == 0 {
                        ...
                        current_config.config.state =
                            format!("distilling-e{}-s{}", epoch + 1, count);
        ...
        if count > 0 {
            ...
            println!(
                "✅ Época {}/{} completada | Loss: {:.4} | Tiempo: {:?} | Total: {:?}",
                epoch + 1,
                epochs,
                avg_loss,
                start.elapsed(),
    ...
    println!(
        "\n✨ Destilación completada o interrumpida en {:?}.",
        global_start.elapsed()
    );
